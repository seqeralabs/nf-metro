...
| `tb_off_track_output_row.mmd`           | `direction: TB` trunk feeding off-track outputs; each output sits beside its own producer's row on the non-label side, not dropped a row down the trunk into a neighbour's label lane (issue #1384) |
