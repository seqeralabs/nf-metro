...
| `stacked_top_entry_flanked_riser.mmd`                  | A row-2 section entered from the section above with a second line running past to a neighbouring column - the riser dropping into the TOP entry runs in the inter-column gap that the neighbour flanks, so the gap widens to seat it a clearance off both walls (#1568) |
| `stacked_top_entry_flanked_riser_crossline.mmd`        | The same flanked TOP-entry riser with a distinct line climbing the same gap and overlapping it in Y, so the two counter-running centrelines settle a bundle clearance apart (#1568)                                                                                     |
