...
This directory holds 294 fixtures and every one of them is named in this file: the illustrated ones in the walkthrough sections below, the rest in the [Regression Catalogue](#regression-catalogue).
...
The 215 fixtures below are targeted regression guards: each was added to pin a
...
| `bypass_left_entry_colspan_intervener.mmd` | A LEFT-entry two-legged bypass over an intervener that reaches the descent corridor only through its `grid_col_span`, not its start column - the wrap band must still be reserved so the run clears the intervening box instead of squeezing through it (#1882)                                  |
...
| `rl_left_exit_top_entry_overflow.mmd`                  | An RL section with a LEFT exit and a TOP entry whose port sits left of the internal run: the drop is predicted to land right of the run, so the run shifts left to meet the port and, without a matching bbox grow, escapes into the neighbouring column (#1980)   |
| `left_exit_fan_perp_entry_landing.mmd`                 | A LEFT-exit two-line bundle dropping into a shared TOP entry whose downstream fan peels each line to a different target - the fan-out's peel order propagates to its free upstream feeder so both lines descend straight into their lanes with no crossing (#1986) |
...
| `entry_fan_root_vs_deeper_arm.mmd`       | One LEFT entry port carrying a line straight to the section root and a second line straight to a deeper arm the root itself feeds - the port seats on the root's row so the root line runs flat into the trunk and the level change becomes a riser in the gap, rather than seating on the deeper arm and doglegging the root line at the boundary (#1771) |
