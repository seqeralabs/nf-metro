...
| `left_exit_fan_perp_entry_landing.mmd`                 | A LEFT-exit two-line bundle dropping into a shared TOP entry whose settled exit-turn axis differs from the per-line crossing lane - the descent tapers onto the crossing lane above the boundary so approach and departure cross at one consistent X (#1986)     |
