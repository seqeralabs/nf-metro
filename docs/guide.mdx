...
When you place sections by hand with `%%metro grid: <section> | <col>,<row>`, each cell normally holds one section, and every cell in a column is as wide as the column's widest section. To pack two short sections into the horizontal space a single wide section would take, name several comma-separated sections in one cell:
...
| `%%metro logo: <path>`                                                     | Logo image, bundled into the legend (or top-left if there is no legend). The path is resolved relative to the `.mmd` file's directory; an unresolvable path is an error.                                                                                                                                                                                                                               |
...
| `%%metro grid: <sections> \| <col>,<row>[,<rowspan>[,<colspan>]]`          | Pin a section to a grid position. The first field may name several comma-separated sections; they share one cell and pack side-by-side along the flow axis, so a short+long pair aligns top-to-bottom with a long+short pair in the same column below it.                                                                                                                                              |
...
| `%%metro track_gap: <pixels>`                                              | Visual gap between adjacent line strokes in a bundle (0–3 px, edge to edge). `0` means lines touch with no gap between them; default is 1 px (the built-in 4 px centre-to-centre minus the 3 px stroke). CLI equivalent `--track-gap`.                                                                                                                                                                 |
