...
| `%%metro output: <path>[, <path>...] [\| <override>...]`                   | Default output path(s) written by `nf-metro render` when no `-o` is given. Comma-separate or repeat the directive for several, each format taken from its extension (`metro_map.svg, metro_map.png`). Paths resolve relative to the `.mmd` file's directory. Anything after a `\|` sets that line's own render options — `animate`, `mode=`, `theme=`, `scale=`, `raster_width=` — so one run can write a static SVG, an animated SVG and a light/dark PNG pair; see [per-output render overrides](/nf-metro/cli/#per-output-render-overrides). `-o` overrides them entirely; with neither, the render falls back to the sibling `<input>.<format>`. |
...
![Terminal: nf-metro validate reports the unresolved id in a yellow Validation warnings panel, then still calls the map valid](assets/img/nf-metro-validate.svg)
...
✓ 12 stations, 11 edges, 1 lines → broken.svg
╭─ Warnings ──────────────────────────────────────────────────────────────╮
...
┌─ Error ─────────────────────────────────────────────────────────────────┐
│ the settled layout violates Tier-A invariants ...                       │
└─────────────────────────────────────────────────────────────────────────┘
```
