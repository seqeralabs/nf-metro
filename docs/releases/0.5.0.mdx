---
title: "v0.5.0"
slug: releases/0.5.0
---

import Metro from "@components/Metro.astro";

_2026-02-24_ · [GitHub release](https://github.com/pinin4fjords/nf-metro/releases/tag/0.5.0) · [Diff](https://github.com/pinin4fjords/nf-metro/compare/0.4.7...0.5.0)

## Diamond fork-joins

Stations that a line passes straight through - splitting to parallel paths and immediately rejoining - now render as flat diamond bubbles rather than a pair of overlapping pills.
Common at QC branch points where multiple tools run in parallel before merging back.

<Metro src="examples/topologies/section_diamond.mmd" mmd={false} />

## Multi-line station labels

Station labels can span multiple lines using `\n`:

```metro
node[First line\nSecond line]
```

<Metro src="tests/fixtures/multiline_labels.mmd" mmd={false} />

## Multiple file icons per station

A single station can carry more than one file icon:

```metro
node[Tool]:::file:output.bam,file:output.bai
```

<Metro src="examples/guide/05b_multi_icons.mmd" mmd={false} />

## Configurable section spacing

`--x-spacing` and `--y-spacing` (and `%%metro x_spacing:` / `%%metro y_spacing:`) now accept any value above safe minimums rather than a small fixed set of presets.

## New pipeline examples

**nf-core/epitopeprediction:**

<Metro src="examples/epitopeprediction.mmd" mmd={false} />

**nf-core/hlatyping:**

<Metro src="examples/hlatyping.mmd" mmd={false} />

## Other improvements

- Animated balls reduced to 1 per track (was 3)
- Dark-mode CSS fix for transparent-background SVGs
- TB section exit port now mirrors the entry-side gap
- Exit/entry port pairs snapped toward downstream stations
- Straight-through diamond fork-joins
- Terminus stations nudged away from passing lines
- Fork/join bubbles widened for labels; labels flip away from exit ports
- Bypass gap2 vertical offset avoids overlap with L-shape routes
- Stations shifted away from perpendicular entry ports
- Exit gap now only applies to single-layer sections
- Fork gap suppressed in single-track sections
