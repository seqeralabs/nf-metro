...
I've been working with a little side project for a few months now driven almost entirely by AI, and it's shown me the limitations of both of these viewpoints. Here's what I've learned and where I think we're going.
...
This is the bit both camps get wrong, in opposite directions. The agent wrote most of nf-metro. At one point it took an 8,800-line layout engine and split it into a tidy subpackage of separate phases. Another week it tore out the entire parser and replaced it with a proper grammar. I reviewed the shape of those changes, the interfaces, the approach. I did not read every line, and I'm not going to pretend I did.
...
You can see the same split in who caught what over the months. I caught the things that needed an eye for the form: the fan of branches that came out lopsided, the line breezing past a station it was meant to serve, the layout that passed every check and still looked off. The AI caught the things that are well-trodden and mechanical: the missing type annotation, the test pointing at something that no longer exists, the function that had quietly swollen to forty-odd branches. That division isn't a coincidence. It's strong exactly where the problem has already been solved a thousand times in its training, and weak exactly where it's novel, which is the half I find interesting anyway.
