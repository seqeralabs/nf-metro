...
**2285/3305** gates fully exercised (both/all arms hit by some fixture); **1020** gates have at least one un-exercised arm.
...
0/76 gates fully exercised.

Gates with an un-exercised arm:

| Line | Gate | Un-exercised arm(s) | Triage |
| ---: | --- | --- | --- |
| 93 | `if (self.anchor_role_id is None) == (self.anchor_coordinate is None):` | `->L-92`, `->L94` |  |
| 265 | `if len(target_keys) != len(set(target_keys)):` | `->L266`, `->L269` |  |
| 270 | `for variable in variables:` | `->L271`, `->L282` |  |
| 272 | `if key in variables_by_segment:` | `->L273`, `->L276` |  |
| 276 | `if variable.axis not in (0, 1) or not isfinite(variable.coordinate):` | `->L277`, `->L280` |  |
| 286 | `for target in targets:` | `->L287`, `->L405` |  |
| 288 | `for segment_rank, (start, end) in enumerate(zip(points, points[1:])):` | `->L286`, `->L289` |  |
| 289 | `if isclose(start[0], end[0], abs_tol=COORD_TOLERANCE):` | `->L290`, `->L291` |  |
| 291 | `elif isclose(start[1], end[1], abs_tol=COORD_TOLERANCE):` | `->L292`, `->L294` |  |
| 296 | `if direction is None:` | `->L297`, `->L298` |  |
| 315 | `if any(item is not None for item in explicit_coordinate_controls):` | `->L316`, `->L335` |  |
| 316 | `if (` | `->L321`, `->L325` |  |
| 326 | `if coordinate_control is None:` | `->L327`, `->L331` |  |
| 338 | `if explicit_start is not None:` | `->L339`, `->L343` |  |
| 346 | `if explicit_end is not None:` | `->L347`, `->L351` |  |
| 351 | `for endpoint_variable in (start_variable, end_variable):` | `->L352`, `->L357` |  |
| 352 | `if endpoint_variable is not None and endpoint_variable.axis != 1 - axis:` | `->L351`, `->L353` |  |
| 357 | `if coordinate_variable is not None and coordinate_variable.axis != axis:` | `->L358`, `->L361` |  |
| 473 | `if (self.variable_id is None) == (self.coordinate is None):` | `->L-472`, `->L474` |  |
| 522 | `if (` | `->L537`, `->L542` |  |
| 547 | `abs(start[axis] - end[axis]) > COORD_TOLERANCE` | `->L548`, `->L550` |  |
| 548 | `or direction is not claim.direction` | `->L550`, `->L553` |  |
| 556 | `if longitudinal_end - longitudinal_start <= COORD_TOLERANCE:` | `->L557`, `->L560` |  |
| 577 | `for target in targets:` | `->L578`, `->L579` |  |
| 579 | `if any(len(items) != 1 for items in by_key.values()):` | `->L580`, `->L583` |  |
| 584 | `for claim in ledger.claims:` | `->L585`, `->L599` |  |
| 585 | `if claim.edge_key is None or claim.family_id is None:` | `->L586`, `->L587` |  |
| 588 | `if len(matches) != 1:` | `->L589`, `->L594` |  |
| 589 | `if claim.reservation_complete:` | `->L590`, `->L593` |  |
| 597 | `if claim.reservation_complete:` | `->L584`, `->L598` |  |
| 610 | `claim.ledger.endpoint_cohort_id is None` | `->L611`, `->L619` |  |
| 611 | `or claim.ledger.destination_boundary_axis_sign is None` | `->L612`, `->L619` |  |
| 612 | `or not claim.ledger.reservation_complete` | `->L613`, `->L619` |  |
| 613 | `or not claim.target.mutable` | `->L614`, `->L619` |  |
| 614 | `or route.exit_turn_plan_id is None` | `->L615`, `->L619` |  |
| 615 | `or route.exit_turn_segment_rank is None` | `->L616`, `->L619` |  |
| 616 | `or claim.ledger.segment_rank == route.exit_turn_segment_rank` | `->L617`, `->L619` |  |
| 617 | `or abs(claim.ledger.segment_rank - route.exit_turn_segment_rank) != 1` | `->L619`, `->L620` |  |
| 637 | `if recipe is None:` | `->L638`, `->L639` |  |
| 639 | `if (` | `->L648`, `->L652` |  |
| 657 | `for point in recipe.controlled_points:` | `->L658`, `->L685` |  |
| 665 | `if (` | `->L679`, `->L683` |  |
| 685 | `for runway in recipe.directed_runways:` | `->L-632`, `->L686` |  |
| 686 | `if (` | `->L685`, `->L697` |  |
| 721 | `for claim in claims:` | `->L722`, `->L727` |  |
| 724 | `if _directly_movable(claim):` | `->L721`, `->L725` |  |
| 729 | `for key in sorted(by_segment):` | `->L730`, `->L764` |  |
| 732 | `if any(` | `->L743`, `->L746` |  |
| 766 | `if len(variable_ids) != len(set(variable_ids)):` | `->L767`, `->L770` |  |
| 770 | `for request in scalar_requests:` | `->L771`, `->L787` |  |
| 776 | `request.domain.member_id != request.variable.variable_id` | `->L777`, `->L780` |  |
| 777 | `or not isfinite(request.preferred_coordinate)` | `->L778`, `->L780` |  |
| 778 | `or any(bound is not None and not isfinite(bound) for bound in bounds)` | `->L780`, `->L784` |  |
| 823 | `for lead in witnesses:` | `->L824`, `->L923` |  |
| 829 | `if lead.coordinate_variable_id is not None or len(endpoint_variables) != 1:` | `->L830`, `->L831` |  |
| 834 | `if controller_carrier is None:` | `->L835`, `->L838` |  |
| 838 | `if controller.axis != 1 - lead.axis:` | `->L839`, `->L842` |  |
| 842 | `if isclose(` | `->L847`, `->L848` |  |
| 848 | `elif isclose(` | `->L853`, `->L855` |  |
| 862 | `if fixed_endpoint > controller.coordinate:` | `->L863`, `->L865` |  |
| 876 | `if endpoint_cohorts_by_variable.get(controller_id):` | `->L877`, `->L878` |  |
| 878 | `for candidate_id, carrier in carrier_by_variable.items():` | `->L823`, `->L879` |  |
| 879 | `if candidate_id == controller_id:` | `->L880`, `->L881` |  |
| 883 | `candidate.axis != controller.axis` | `->L884`, `->L891` |  |
| 884 | `or not endpoint_cohorts_by_variable.get(candidate_id)` | `->L885`, `->L891` |  |
| 885 | `or candidate.edge_key[2] == lead.line_id` | `->L886`, `->L891` |  |
| 886 | `or candidate.member_id == lead.member_id` | `->L887`, `->L891` |  |
| 887 | `or candidate.owner_kind is not CorridorScalarOwnerKind.MEMBER_CARRIER` | `->L888`, `->L891` |  |
| 888 | `or controller.owner_kind is not CorridorScalarOwnerKind.MEMBER_CARRIER` | `->L889`, `->L891` |  |
| 889 | `or carrier.direction is not controller_carrier.direction` | `->L891`, `->L892` |  |
| 892 | `if not (` | `->L897`, `->L898` |  |
| 899 | `if not (` | `->L902`, `->L903` |  |
| 903 | `if fixed_endpoint > controller.coordinate:` | `->L904`, `->L906` |  |
| 930 | `for variable_id, carrier in carrier_by_variable.items():` | `->L931`, `->L958` |  |
| 932 | `for witness in fixed:` | `->L930`, `->L933` |  |
| 933 | `if (` | `->L944`, `->L945` |  |
