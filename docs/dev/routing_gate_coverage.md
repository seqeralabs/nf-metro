...
212/399 gates fully exercised.

Gates with an un-exercised arm:

| Line | Gate | Un-exercised arm(s) | Triage |
| ---: | --- | --- | --- |
| 214 | `if edge_rank is None:` | `->L215` |  |
| 296 | `if owned:` | `->L290` |  |
| 312 | `if route is None:` | `->L313` |  |
| 333 | `if route is None:` | `->L334` |  |
| 347 | `if family is None:` | `->L348` |  |
| 356 | `if not runs:` | `->L357` |  |
| 378 | `if axis.axis is not DemandAxis.X:` | `->L379` |  |
| 395 | `if route.exit_lane_transition_plan_id is not None:` | `->L396` |  |
| 428 | `if not candidates:` | `->L429` |  |
| 447 | `if vertical and abs(endpoint[0] - target[0]) <= COORD_TOLERANCE:` | `->L450` |  |
| 451 | `if elbow != endpoint and elbow != target:` | `->L452`, `->L453` |  |
| 457 | `if route.offset_regime is OffsetRegime.DEFERRED:` | `->L458` |  |
| 471 | `for rank, (start, end) in enumerate(zip(route.points, route.points[1:])):` | `->L489` |  |
| 474 | `runway <= COORD_TOLERANCE` | `->L477` |  |
| 483 | `if abs(prior[0] - start[0]) + abs(prior[1] - start[1]) > COORD_TOLERANCE:` | `->L488` |  |
| 485 | `if direction_axis(incoming) is not direction_axis(approach):` | `->L488` |  |
| 538 | `if exit_turn_geometry != _exit_turn_geometry(route):` | `->L539` |  |
| 550 | `if approach is None:` | `->L551` |  |
| 603 | `if direction in {Direction.R, Direction.L}:` | `->L607` |  |
| 632 | `if len(route.points) < 2:` | `->L633` |  |
| 642 | `if axis is DemandAxis.X:` | `->L648` |  |
| 643 | `if abs(start[1] - end[1]) > COORD_TOLERANCE:` | `->L644` |  |
| 648 | `if abs(start[0] - end[0]) > COORD_TOLERANCE:` | `->L649`, `->L650` |  |
| 652 | `if extent_end - extent_start > COORD_TOLERANCE:` | `->L631` |  |
| 738 | `if primary_landing is None or trunk_axis.axis is not DemandAxis.X:` | `->L739` |  |
| 767 | `if axis is DemandAxis.X:` | `->L769` |  |
| 817 | `if any(edge not in edge_order for edge in edges):` | `->L818` |  |
| 853 | `if not incoming_edges or not outgoing_edges:` | `->L854` |  |
| 859 | `if group.line_id not in line_order:` | `->L860` |  |
| 876 | `if trunk_edge_key is None:` | `->L877` |  |
| 925 | `if route is not None:` | `->L929` |  |
| 929 | `if route is None:` | `->L930` |  |
| 946 | `if any(` | `->L950` |  |
| 964 | `if (` | `->L969` |  |
| 969 | `for edge_key in incoming_edges:` | `->L970`, `->L973` |  |
| 1027 | `if _shared_terminal_landing_drops_exit_turn(` | `->L1032` | **needs-review** -- The shared-terminal convergence declines ownership when seating its carrier would strand the carrier's own exit turn (issue #1949). tests/fixtures/topologies/convergence_shared_terminal_exit_turn.mmd takes the decline arm, but the gate sweep globs tests/fixtures/ non-recursively, so the fixture under its topologies/ subdirectory is not evaluated here; the render-diff and the dedicated regression tests own it instead. |
| 1128 | `if (` | `->L1140` |  |
| 1172 | `if continuation.covered_by_member_id is not None:` | `->L1173` |  |
| 1281 | `if topology is None:` | `->L1282` |  |
| 1369 | `if plan.trunk_axis is None:` | `->L1370` | **defensive** -- _turn_crosses_shared_run's no-trunk-axis skip, a gate the restored conflict conditions added whose ordinal collided with a pre-existing baseline key, so the ratchet acknowledged it without a verdict. Its sole caller is _system_conflict's trunk-pair scan, whose `trunks` tuple is built only from plans with a trunk axis, so the plan handed over always has one. |
| 1400 | `if _points_coincide(*segment):` | `->L1401` |  |
| 1425 | `if (` | `->L1433` | **needs-review** -- _opposing_landing_approaches' SHARED_APPROACH_CHANNEL condition: two feeders of one line whose approach runs stand on one coordinate travelling against each other. The loop reaches this pair test on 17 fixtures, so the shape of the scan is exercised. Neutralising _settle_opposing_landing_channels makes it fire on merge_bottom_row_bypass and merge_feeder_shared_channel_gap, at separation 0 with 150px of overlap, so that pass alone is what keeps it silent; the two opening passes account for none of it. Reachable. Reaching it is now a feasibility refusal rather than a change of emitter -- see #1712. |
| 1468 | `or abs(start[1] - end[1]) > COORD_TOLERANCE` | `->L1475` |  |
| 1471 | `if direction_axis(incoming) is not landing.approach_axis:` | `->L1475` |  |
| 1580 | `if opening_follows_flank:` | `->L1581` |  |
| 1642 | `if landing.opening_turn_segment is None:` | `->L1643` |  |
| 1645 | `if landing.approach_axis is DemandAxis.X:` | `->L1649` |  |
| 1647 | `if runway < curve_radius - COORD_TOLERANCE:` | `->L1648` |  |
| 1678 | `landing.approach_axis is DemandAxis.X` | `->L1682` | **needs-review** -- _reseat_landing_cross moves a stated opening turn only on an X approach, because the opening turn names a column and _landing_cross_segment reads a perpendicular approach's crossing row off the runway whatever opening turn the landing carries. The false arm is the perpendicular class the widened _settle_opposing_landing_channels scope newly admits; corpus-wide every counter-running same-line pair that conflicts approaches on X. Reachable. |
| 1679 | `and landing.opening_turn_segment is not None` | `->L1682` | **defensive** -- The second operand of the same condition. A landing that states an opening turn coordinate states its segment with it, so on an X approach the operand is true wherever the first is; it narrows the Optional _reseat_landing_opening requires. |
| 1684 | `if runway < curve_radius - COORD_TOLERANCE:` | `->L1685`, `->L1686` | **needs-review** -- _reseat_landing_cross refusing a seat that costs the approach corner its radius, on the runway-stated branch. Un-exercised because that branch is itself un-exercised; see `if landing.opening_turn_segment is not None:::#1`. Reachable. |
| 1698 | `for rank, landing in enumerate(landings):` | `->L1699`, `->L1706` |  |
| 1699 | `if landing.member_id not in member_ids:` | `->L1700`, `->L1701` |  |
| 1702 | `if reseated is None:` | `->L1703`, `->L1704` |  |
| 1739 | `if opening_segment is not None:` | `->L1748` |  |
| 1957 | `if any(` | `->L1962` | **needs-review** -- _packed_lane's decline arm: packing a run onto the pitch of its nearest bundle neighbour would seat it inside a *second* neighbour's pitch. Reachable -- 3-lane bundles exist in the corpus (_packed_lane sees len(neighbours)==2 on examples/guide/03b_fan_in_merge.mmd) -- but no fixture makes the candidate breach the far lane. Wants a fixture. |
| 2019 | `if axis is None or axis.axis is not DemandAxis.X:` | `->L2020` |  |
| 2057 | `if plan.edge in owned_edges:` | `->L2058` | **defensive** -- _member_corridor_runs skips a frozen member plan whose edge a convergence already owns, but the two sets are disjoint by construction: build_member_geometry_execution never freezes a plan for an edge in _convergence_member_edges (member_geometry.py:1010), and resolved_member_edges is derived from the same scaffold predicate. 3830 frozen member plans across 348 fixtures, zero hits. |
| 2066 | `or abs(before[0] - start[0]) > COORD_TOLERANCE` | `->L2069` | **needs-review** -- Rejects an interior horizontal whose leading flank is not vertical. Reachable: _member_corridor_runs walks raw plan.points with no normalize filter, and the corpus already freezes an H-D-H member polyline (examples/topologies/rail_boundary_bundle_fan.mmd), so one extra leg makes a horizontal run diagonal-flanked. Not the same contract as the look-alike defensive pair in common.py::iter_horizontal_trunks, which is fed only normalize-filtered bypass U-shapes. Wants a fixture. |
| 2067 | `or abs(after[0] - end[0]) > COORD_TOLERANCE` | `->L2069` | **needs-review** -- Mirror of the leading-flank reject: an interior horizontal whose trailing flank is not vertical. Same H-D-H evidence and the same reason it is not covered by the common.py precedent. Wants a fixture. |
| 2109 | `if run is None:` | `->L2110` | **defensive** -- _pack_cotravelling_corridor_runs skips a plan with no X-axis trunk run. _trunk_corridor_run returns None only when trunk_axis is absent or is DemandAxis.Y; every plan reaching here is owns_geometry, and owns_geometry => trunk_axis is not None is asserted at four call sites. One root cause with the module's other DemandAxis.X scope guards: no corpus topology produces a vertical convergence trunk (346 calls, all X). |
| 2168 | `if run is None:` | `->L2169` |  |
| 2201 | `if axis is None:` | `->L2202` |  |
| 2243 | `if axis is None or axis.axis is not DemandAxis.X:` | `->L2244` |  |
| 2253 | `if primary is None:` | `->L2254` |  |
| 2266 | `or landing.opening_turn_segment is None` | `->L2268` |  |
| 2279 | `if reseated is not None:` | `->L2262` |  |
| 2330 | `obstacle_segment is None` | `->L2335` | **defensive** -- A resident is read back from `settled` on every comparison, and laning moves its cross run rather than removing it, so the segment is drawable whenever it is read. The operand guards the Optional that read returns. |
| 2331 | `or landing_segment is None` | `->L2335` |  |
| 2346 | `if reseated is None:` | `->L2347` |  |
| 2352 | `landing.member_id == plan.primary_trunk_member_id` | `->L2375` | **defensive** -- The first conjunct of the widened flank-carry condition. Its false arm is the ordinary feeder landing, which every fixture re-seating a landing takes; the coverage tool reports the operand separately from the conjunction above it. |
| 2353 | `and plan.trunk_axis is not None` | `->L2375` | **defensive** -- _settle_opposing_landing_channels carries the trunk flank only when the landing it re-seated is the plan's primary trunk member. A plan naming a primary trunk member always states the trunk axis that member travels, so the false arm is unreachable through this conjunction; it narrows the Optional for the flank move below. |
| 2354 | `and plan.trunk_axis.axis is landing.approach_axis` | `->L2375` | **needs-review** -- The flank follows a re-seated landing only where flank and landing are measured on one axis. Every primary-trunk landing the corpus re-seats approaches on its own trunk's axis, so the false arm wants a fixture whose trunk travels one axis while its primary trunk member's landing approaches on the other -- a perpendicular approach into a convergence, which is the class the widened scope newly admits. Reachable. |
| 2384 | `if (` | `->L2317` | **defensive** -- A landing enters `ordered` only because _landing_cross_segment admitted it, and laning moves that run rather than removing it, so the read that names the landing as a resident always finds a segment. The gate guards the Optional that read returns. |
| 2399 | `if axis is None:` | `->L2400` |  |
| 2423 | `if coordinate is not None:` | `->L2428` |  |
| 2465 | `if endpoint is None:` | `->L2466` |  |
| 2488 | `if plan.trunk_axis is None:` | `->L2489` | **defensive** -- Pre-existing gap, re-keyed from #1 by the restored _turn_crosses_shared_run guard that now takes that ordinal (unchanged [0,11] split). _lane_trunk_flanks' skip for a plan with no trunk axis. Every plan reaching it is PLANNED -- the two _settle_convergence_geometry call sites filter on owns_geometry and the third caller passes freshly built planned plans -- and validate_convergence_plans asserts trunk_axis is not None for exactly those, so a plan that took this skip would abort on that assertion rather than render. _build_planned_convergence can construct such a plan, so the guard is defensive in practice rather than unconstructible. |
| 2514 | `if coordinate is not None:` | `->L2520` | **needs-review** -- Pre-existing gap, re-keyed from #2 by the newly inserted packing gates (unchanged [0,2] split). _nearest_lane's fall-through when no flank candidate clears the runway: it runs 9 times, always against a single obstacle, and in 5 of those one of the two candidates is already rejected by the curve-radius runway test, so a shorter flank runway rejects both. Reachable; wants a fixture. |
| 2574 | `for seat, obstacle in obstacles:` | `->L-2559`, `->L2575` | **needs-review** -- The give-way loop in _lane_trunk_flanks, entered only when no lane one clearance from the residents leaves the arriving flank a full turn radius to its endpoint. No corpus fixture crowds two same-line counter-running flanks that tightly, so the loop body is un-entered. Reachable, and exercised by test_a_flank_with_no_lane_of_its_own_is_given_way_to_by_the_resident and by test_a_resident_gives_way_to_a_lane_clear_of_the_flanks_around_it. |
| 2576 | `if axis is None:` | `->L2577`, `->L2578` |  |
| 2588 | `if lane is None:` | `->L2589`, `->L2590` | **needs-review** -- A resident asked to give way that has no reachable lane of its own either. Un-entered because the give-way path itself is un-entered on the corpus. Reachable; a fixture is a channel with no seat for either run, which is the population #1712 exists to give room to. |
| 2609 | `if flank_rank != 1:` | `->L2611` |  |
| 2620 | `if fork is None:` | `->L2621`, `->L2629` |  |
| 2697 | `if forked:` | `->L2698` |  |
| 2698 | `if abs(flank_coordinate - landing_coordinate) <= COORD_TOLERANCE:` | `->L2699`, `->L2700` |  |
| 2702 | `if abs(endpoint - flank_coordinate) <= clearance:` | `->L2703` |  |
| 2726 | `if forked:` | `->L2727`, `->L2728` | **needs-review** -- _landing_gives_way_to_flank joins the landing to the flank's column when the two are one stroke off a shared fork, and steps it one clearance off otherwise. Neither arm is reached because _flank_settled_column never declines on the corpus: all 32 crowded landing/flank pairs seat with 9px to spare. Reachable; a fixture needs an inter-column gap below MERGE_GAP_MIN carrying a chained convergence. |
| 2801 | `if endpoint is None:` | `->L2802` | **defensive** -- _flank_lane needs the endpoint its flank turns onto to price the runway. ConvergenceTrunkAxis states source and target endpoint coordinates together or not at all, and every construction site in this module passes both, so the arm guards the Optional rather than a shape the planner builds. |
| 2812 | `if coordinate is None:` | `->L2813` |  |
| 2825 | `if reseated is not None:` | `->L2826`, `->L2829` |  |
| 2890 | `if not (` | `->L2893` |  |
| 2909 | `if axis is None or axis.axis is not DemandAxis.X:` | `->L2910` | **defensive** -- Pre-existing gap, re-keyed from #2 by the newly inserted packing gates (unchanged [0,18] split). The `axis is None` half cannot fire: callers filter on plan.owns_geometry and owns_geometry => trunk_axis is not None is asserted at 4 sites. The DemandAxis.Y half is unsettled -- a vertical trunk axis is constructible in code (_direct_axis_points / _shared_terminal_axis yield Y for a vertical approach) but no corpus topology produces one; if a TB convergence trunk is shown constructible this reclassifies reachable-but-benign (the guard returns the right empty answer). |
| 2967 | `if primary_edge is not None and plan.primary_trunk_member_id is not None:` | `->L2952` |  |
| 3174 | `if not carries_coupled_primary and any(` | `->L3180` | **needs-review** -- Reached by the corpus at 897747b0 and no longer: the branch's later commits (908637eb..04209e3d) changed which paths fire, so this arm lost its last covering fixture rather than becoming unreachable. Left un-exercised deliberately -- the render corpus is frozen for a measurement against main, so no fixture was authored. |
| 3186 | `if endpoint is None or abs(endpoint - column) <= curve_radius:` | `->L3187` |  |
| 3204 | `if (endpoint - candidate) * toward_endpoint <= curve_radius:` | `->L3205` |  |
| 3259 | `if axis is None:` | `->L3260` | **defensive** -- A resident flank in _give_way_to was seated from a plan whose trunk axis was read to build it, so the plan still states one. The guard narrows the Optional before the flank move. |
| 3403 | `if plan.trunk_axis is None or plan.trunk_axis.axis is not DemandAxis.X:` | `->L3404` |  |
| 3485 | `for candidate_coordinate in direct_candidates:` | `->L3511` | **needs-review** -- Reached by the corpus at 897747b0 and no longer: the branch's later commits (908637eb..04209e3d) changed which paths fire, so this arm lost its last covering fixture rather than becoming unreachable. Left un-exercised deliberately -- the render corpus is frozen for a measurement against main, so no fixture was authored. |
| 3496 | `if not moved_channels or any(` | `->L3501` | **needs-review** -- Reached by the corpus at 897747b0 and no longer: the branch's later commits (908637eb..04209e3d) changed which paths fire, so this arm lost its last covering fixture rather than becoming unreachable. Left un-exercised deliberately -- the render corpus is frozen for a measurement against main, so no fixture was authored. |
| 3502 | `if any(` | `->L3506` | **needs-review** -- Reached by the corpus at 897747b0 and no longer: the branch's later commits (908637eb..04209e3d) changed which paths fire, so this arm lost its last covering fixture rather than becoming unreachable. Left un-exercised deliberately -- the render corpus is frozen for a measurement against main, so no fixture was authored. |
| 3507 | `if not landing_feasible(candidate_coordinate):` | `->L3508` |  |
| 3511 | `if coordinate is None:` | `->L3512` | **needs-review** -- Reached by the corpus at 897747b0 and no longer: the branch's later commits (908637eb..04209e3d) changed which paths fire, so this arm lost its last covering fixture rather than becoming unreachable. Left un-exercised deliberately -- the render corpus is frozen for a measurement against main, so no fixture was authored. |
| 3517 | `if (` | `->L3521` |  |
| 3540 | `if not obstacles:` | `->L3542` | **defensive** -- The ordinal moved when the endpoint check left this condition; the arm is the ordinary no-crowding case _lane_trunk_flanks takes on most plans. |
| 3548 | `for coordinate in candidates:` | `->L3527`, `->L3549` |  |
| 3549 | `if not _inside_usable_gap(gap_edges, coordinate):` | `->L3550`, `->L3551` |  |
| 3557 | `if moved_plan is None:` | `->L3558`, `->L3559` |  |
| 3568 | `if moved_channel is None or any(` | `->L3572`, `->L3573` |  |
...
| 3594 | `if first.system_id != second.system_id:` | `->L3595` |  |
| 3596 | `if first.claimant_member_ids & second.claimant_member_ids:` | `->L3597` | **needs-review** -- _channels_share_source_carrier's shared-claimant shortcut. Its sole caller is _validate_final_convergence_feasibility, which pairs a convergence plan channel against a frozen member channel; the intersection is non-empty only when the plan's channel names the very member it is compared with, which no corpus fixture produces. Reachable in principle, wants a fixture. |
| 3603 | `if first.member_geometry_owned == second.member_geometry_owned:` | `->L3604` | **needs-review** -- _channels_share_source_carrier's equal-ownership rejection. Its sole caller always pairs a convergence-owned channel with a member-owned one, so the two flags always differ and the rejection arm is unreachable from that caller. Reachable only if a second caller compares two channels of one owner. |
| 3777 | `if _direction(*landing_segment) not in (Direction.U, Direction.D):` | `->L3778` |  |
| 3782 | `if endpoint is None or _shared_source_bundle_stroke(` | `->L3790` |  |
| 3795 | `if shortfall <= SAME_COORD_TOLERANCE:` | `->L3797` |  |
| 3810 | `if gap is None:` | `->L3811`, `->L3812` |  |
| 3832 | `if not negative or not positive:` | `->L3833`, `->L3834` |  |
| 3860 | `if current is None or requirement.required > current.required:` | `->L3769`, `->L3861` |  |
| 3908 | `if first_plan.trunk_axis.axis is DemandAxis.X:` | `->L3912` | **needs-review** -- The Y-axis half of the opening-arm delta measurement. Six fixtures reach it and all take the X half, for the one root cause shared with this module's other DemandAxis.X scope guards: no corpus topology produces a vertical convergence trunk. Not a guard -- both halves are correct code for their axis -- so it is reachable exactly when a vertical trunk axis is. What would settle it is a TB convergence whose _direct_axis_points / _shared_terminal_axis yields DemandAxis.Y. |
| 3915 | `if first_delta * second_delta < 0:` | `->L3918` | **needs-review** -- OPPOSING_OPENING_CHANNEL: two arms of one line that turn on one shared column and then open to opposite sides of their source. Six fixtures reach the test with a genuine shared-opening pair, and in every one the two trunks open the same way, which the code below reads as one stroke branching to several destinations. Neutralising _settle_opposing_landing_channels makes it fire on merge_bottom_row_bypass and merge_feeder_shared_channel_gap; neutralising _settle_shared_opening_pivots and _settle_shared_source_openings does not, and the pivot pass cannot suppress it in any case because its group key partitions by the travel direction of flank 1. Reachable; reaching it is a feasibility refusal rather than a change of emitter -- see #1712. |
| 3932 | `if opposing_approaches is not None:` | `->L3933` | **needs-review** -- The refusal arm for the SHARED_APPROACH_CHANNEL conflict _opposing_landing_approaches reports. Un-exercised for exactly the reason that helper's own pair test is (`convergences.py::if (::#4`): the helper is called on 18 fixtures and returns None on every one. Reachable; closing it and closing #4 are the same fixture. |
| 3977 | `if same_line and first_direction is not second_direction:` | `->L3982` | **needs-review** -- SHARED_TRUNK_CHANNEL for a same-line pair whose outward and return legs crowd one channel and which _settle_shared_trunk_channels could seat no lane for: both runs boxed in by the obstacles either side of them, since the arriving flank now gives way to a resident and the resident gives way back. Read in the travel frame the laning pass decides in, so it no longer fires on a rank-1/rank-3 pair that pass deliberately fused. Neutralising that pass makes it fire 5 times on merge_around_below_leftmost, all central-run pairs at separation 0 and 6px. Reachable; reaching it is a feasibility refusal rather than a change of emitter -- see #1712. |
| 3983 | `if (` | `->L4010` |  |
| 4030 | `if conflict is not None:` | `->L4031` |  |
| 4039 | `if requirements:` | `->L4040`, `->L4042` |  |
| 4067 | `if shared_carrier and separation > COORD_TOLERANCE:` | `->L4068` |  |
| 4074 | `if (` | `->L4085` | **defensive** -- Pre-existing gap, re-keyed from #11 by the gates the restored conflict conditions inserted earlier in the file (unchanged [0,18] split). validate_convergence_plans' re-check that a landing's join point sits on its plan's own trunk axis and on the trunk route's polyline. The join point is derived from the trunk axis when the plan is built and consume_convergence_route connects the emitted endpoint to it, so taking the raise arm means the planner contradicted its own axis: an engine bug and a hard abort, not a shippable topology. |
| 4092 | `if _gap_channels_crowd(` | `->L4099` |  |
| 4115 | `if not plan.owns_geometry:` | `->L4116` |  |
| 4179 | `if not plan.owns_geometry:` | `->L4180` |  |
| 4202 | `owner_edge not in edge_ranks` | `->L4206` |  |
| 4241 | `if not plan.owns_geometry:` | `->L4242` |  |
| 4260 | `if ownership.edge in by_edge:` | `->L4261` |  |
| 4385 | `for item in system_plans:` | `->L4386`, `->L4396` |  |
| 4654 | `if not horizontal and not vertical:` | `->L4655` |  |
| 4678 | `if interval[1] - interval[0] > COORD_TOLERANCE:` | `->L4663` |  |
| 4681 | `for interval_start, interval_end in sorted(intervals):` | `->L4689` |  |
| 4684 | `if interval_start > covered_until + COORD_TOLERANCE:` | `->L4685` |  |
| 4687 | `if covered_until >= extent_end - COORD_TOLERANCE:` | `->L4681` |  |
| 4711 | `if axis.axis is DemandAxis.X:` | `->L4734` |  |
| 4829 | `if axis.direction in {Direction.L, Direction.U}:` | `->L4830`, `->L4831` | **needs-review** -- `convergence_corridor_requests` and its helpers are #2016's request producer: exposed and unit-tested (`tests/test_convergence_corridor_grants.py`) but not yet called from `routing/planning.py` or any other production path, so no corpus fixture can reach this arm. #2017 completes the control-recipe wiring for this program (#1745); reconcile this entry then per docs/dev/routing_gate_triage.md's 'Reconcile needs-review when a parked bug closes' lifecycle -- replace with a fixture if the wiring makes the arm reachable through a real render, or reclassify if it does not. |
| 4839 | `for run in runs:` | `->L4840`, `->L4845` | **needs-review** -- `convergence_corridor_requests` and its helpers are #2016's request producer: exposed and unit-tested (`tests/test_convergence_corridor_grants.py`) but not yet called from `routing/planning.py` or any other production path, so no corpus fixture can reach this arm. #2017 completes the control-recipe wiring for this program (#1745); reconcile this entry then per docs/dev/routing_gate_triage.md's 'Reconcile needs-review when a parked bug closes' lifecycle -- replace with a fixture if the wiring makes the arm reachable through a real render, or reclassify if it does not. |
| 4840 | `if not _points_coincide(points[-1], run[0]):` | `->L4841`, `->L4844` | **needs-review** -- `convergence_corridor_requests` and its helpers are #2016's request producer: exposed and unit-tested (`tests/test_convergence_corridor_grants.py`) but not yet called from `routing/planning.py` or any other production path, so no corpus fixture can reach this arm. #2017 completes the control-recipe wiring for this program (#1745); reconcile this entry then per docs/dev/routing_gate_triage.md's 'Reconcile needs-review when a parked bug closes' lifecycle -- replace with a fixture if the wiring makes the arm reachable through a real render, or reclassify if it does not. |
| 4875 | `if axis is None or plan.primary_trunk_member_id is None:` | `->L4876`, `->L4877` | **needs-review** -- `convergence_corridor_requests` and its helpers are #2016's request producer: exposed and unit-tested (`tests/test_convergence_corridor_grants.py`) but not yet called from `routing/planning.py` or any other production path, so no corpus fixture can reach this arm. #2017 completes the control-recipe wiring for this program (#1745); reconcile this entry then per docs/dev/routing_gate_triage.md's 'Reconcile needs-review when a parked bug closes' lifecycle -- replace with a fixture if the wiring makes the arm reachable through a real render, or reclassify if it does not. |
| 4885 | `if ownership is None:` | `->L4886`, `->L4887` | **needs-review** -- `convergence_corridor_requests` and its helpers are #2016's request producer: exposed and unit-tested (`tests/test_convergence_corridor_grants.py`) but not yet called from `routing/planning.py` or any other production path, so no corpus fixture can reach this arm. #2017 completes the control-recipe wiring for this program (#1745); reconcile this entry then per docs/dev/routing_gate_triage.md's 'Reconcile needs-review when a parked bug closes' lifecycle -- replace with a fixture if the wiring makes the arm reachable through a real render, or reclassify if it does not. |
| 4889 | `if edge is None:` | `->L4890`, `->L4891` | **needs-review** -- `convergence_corridor_requests` and its helpers are #2016's request producer: exposed and unit-tested (`tests/test_convergence_corridor_grants.py`) but not yet called from `routing/planning.py` or any other production path, so no corpus fixture can reach this arm. #2017 completes the control-recipe wiring for this program (#1745); reconcile this entry then per docs/dev/routing_gate_triage.md's 'Reconcile needs-review when a parked bug closes' lifecycle -- replace with a fixture if the wiring makes the arm reachable through a real render, or reclassify if it does not. |
| 4909 | `if source_collapsed:` | `->L4910`, `->L4911` | **needs-review** -- `convergence_corridor_requests` and its helpers are #2016's request producer: exposed and unit-tested (`tests/test_convergence_corridor_grants.py`) but not yet called from `routing/planning.py` or any other production path, so no corpus fixture can reach this arm. #2017 completes the control-recipe wiring for this program (#1745); reconcile this entry then per docs/dev/routing_gate_triage.md's 'Reconcile needs-review when a parked bug closes' lifecycle -- replace with a fixture if the wiring makes the arm reachable through a real render, or reclassify if it does not. |
| 4912 | `if target_collapsed:` | `->L4913`, `->L4914` | **needs-review** -- `convergence_corridor_requests` and its helpers are #2016's request producer: exposed and unit-tested (`tests/test_convergence_corridor_grants.py`) but not yet called from `routing/planning.py` or any other production path, so no corpus fixture can reach this arm. #2017 completes the control-recipe wiring for this program (#1745); reconcile this entry then per docs/dev/routing_gate_triage.md's 'Reconcile needs-review when a parked bug closes' lifecycle -- replace with a fixture if the wiring makes the arm reachable through a real render, or reclassify if it does not. |
| 4932 | `if source_collapsed:` | `->L4933`, `->L4934` | **needs-review** -- `convergence_corridor_requests` and its helpers are #2016's request producer: exposed and unit-tested (`tests/test_convergence_corridor_grants.py`) but not yet called from `routing/planning.py` or any other production path, so no corpus fixture can reach this arm. #2017 completes the control-recipe wiring for this program (#1745); reconcile this entry then per docs/dev/routing_gate_triage.md's 'Reconcile needs-review when a parked bug closes' lifecycle -- replace with a fixture if the wiring makes the arm reachable through a real render, or reclassify if it does not. |
| 4934 | `if target_collapsed:` | `->L4935`, `->L4936` | **needs-review** -- `convergence_corridor_requests` and its helpers are #2016's request producer: exposed and unit-tested (`tests/test_convergence_corridor_grants.py`) but not yet called from `routing/planning.py` or any other production path, so no corpus fixture can reach this arm. #2017 completes the control-recipe wiring for this program (#1745); reconcile this entry then per docs/dev/routing_gate_triage.md's 'Reconcile needs-review when a parked bug closes' lifecycle -- replace with a fixture if the wiring makes the arm reachable through a real render, or reclassify if it does not. |
| 4977 | `if band is None:` | `->L4978`, `->L4979` | **needs-review** -- `convergence_corridor_requests` and its helpers are #2016's request producer: exposed and unit-tested (`tests/test_convergence_corridor_grants.py`) but not yet called from `routing/planning.py` or any other production path, so no corpus fixture can reach this arm. #2017 completes the control-recipe wiring for this program (#1745); reconcile this entry then per docs/dev/routing_gate_triage.md's 'Reconcile needs-review when a parked bug closes' lifecycle -- replace with a fixture if the wiring makes the arm reachable through a real render, or reclassify if it does not. |
| 4982 | `if minimum_coordinate is not None:` | `->L4983`, `->L4984` | **needs-review** -- `convergence_corridor_requests` and its helpers are #2016's request producer: exposed and unit-tested (`tests/test_convergence_corridor_grants.py`) but not yet called from `routing/planning.py` or any other production path, so no corpus fixture can reach this arm. #2017 completes the control-recipe wiring for this program (#1745); reconcile this entry then per docs/dev/routing_gate_triage.md's 'Reconcile needs-review when a parked bug closes' lifecycle -- replace with a fixture if the wiring makes the arm reachable through a real render, or reclassify if it does not. |
| 4984 | `if maximum_coordinate is not None:` | `->L4985`, `->L4986` | **needs-review** -- `convergence_corridor_requests` and its helpers are #2016's request producer: exposed and unit-tested (`tests/test_convergence_corridor_grants.py`) but not yet called from `routing/planning.py` or any other production path, so no corpus fixture can reach this arm. #2017 completes the control-recipe wiring for this program (#1745); reconcile this entry then per docs/dev/routing_gate_triage.md's 'Reconcile needs-review when a parked bug closes' lifecycle -- replace with a fixture if the wiring makes the arm reachable through a real render, or reclassify if it does not. |
| 5003 | `if axis.axis is DemandAxis.X:` | `->L5004`, `->L5006` | **needs-review** -- `convergence_corridor_requests` and its helpers are #2016's request producer: exposed and unit-tested (`tests/test_convergence_corridor_grants.py`) but not yet called from `routing/planning.py` or any other production path, so no corpus fixture can reach this arm. #2017 completes the control-recipe wiring for this program (#1745); reconcile this entry then per docs/dev/routing_gate_triage.md's 'Reconcile needs-review when a parked bug closes' lifecycle -- replace with a fixture if the wiring makes the arm reachable through a real render, or reclassify if it does not. |
| 5031 | `if len(set(owner_ids)) != len(owner_ids):` | `->L5032`, `->L5035` | **needs-review** -- `convergence_corridor_requests` and its helpers are #2016's request producer: exposed and unit-tested (`tests/test_convergence_corridor_grants.py`) but not yet called from `routing/planning.py` or any other production path, so no corpus fixture can reach this arm. #2017 completes the control-recipe wiring for this program (#1745); reconcile this entry then per docs/dev/routing_gate_triage.md's 'Reconcile needs-review when a parked bug closes' lifecycle -- replace with a fixture if the wiring makes the arm reachable through a real render, or reclassify if it does not. |
| 5037 | `for plan in eligible:` | `->L5038`, `->L5057` | **needs-review** -- `convergence_corridor_requests` and its helpers are #2016's request producer: exposed and unit-tested (`tests/test_convergence_corridor_grants.py`) but not yet called from `routing/planning.py` or any other production path, so no corpus fixture can reach this arm. #2017 completes the control-recipe wiring for this program (#1745); reconcile this entry then per docs/dev/routing_gate_triage.md's 'Reconcile needs-review when a parked bug closes' lifecycle -- replace with a fixture if the wiring makes the arm reachable through a real render, or reclassify if it does not. |
| 5039 | `if exposed is None:` | `->L5040`, `->L5043` | **needs-review** -- `convergence_corridor_requests` and its helpers are #2016's request producer: exposed and unit-tested (`tests/test_convergence_corridor_grants.py`) but not yet called from `routing/planning.py` or any other production path, so no corpus fixture can reach this arm. #2017 completes the control-recipe wiring for this program (#1745); reconcile this entry then per docs/dev/routing_gate_triage.md's 'Reconcile needs-review when a parked bug closes' lifecycle -- replace with a fixture if the wiring makes the arm reachable through a real render, or reclassify if it does not. |
| 5079 | `if not candidates:` | `->L5080` |  |
| 5187 | `if not 0 <= flank_rank < len(route.points) - 1:` | `->L5188` |  |
| 5206 | `if actual is None:` | `->L5207` |  |
| 5213 | `direction is not landing.approach_direction` | `->L5217` |  |
| 5214 | `or handedness is not landing.corner_handedness` | `->L5217` |  |
| 5215 | `or runway < landing.minimum_runway - COORD_TOLERANCE` | `->L5217` |  |
| 5234 | `opening is None` | `->L5238` |  |
| 5235 | `or abs(opening.x - landing.opening_turn_coordinate) > COORD_TOLERANCE` | `->L5238` |  |
| 5236 | `or emitted_segment != landing.opening_turn_segment` | `->L5238` |  |
| 5247 | `if query is None:` | `->L5248` |  |
| 5253 | `if not plan.owns_geometry:` | `->L5254` |  |
| 5263 | `if (` | `->L5277` | **defensive** -- Pre-existing gap, re-keyed from #12 by the gates the restored conflict conditions inserted earlier in the file (unchanged [0,18] split). The raise for a covered continuation absent from the carrier member that is recorded as covering it. _reconcile_continuation_ownership only names a carrier whose route already spans the continuation's endpoints, so the raise arm reports an engine bug and aborts the render; no valid topology reaches it. |
| 5281 | `if plan.primary_trunk_member_id == membership.member_id:` | `->L5286` |  |
| 5304 | `if opening is None:` | `->L5305` |  |
| 5305 | `if not clearance_pending:` | `->L5306`, `->L5312` |  |
| 5329 | `if ctx.validate_final_route_frames and not clearance_pending:` | `->L5331` |  |
| 5346 | `if not clearance_pending and any(` | `->L5350` | **needs-review** -- The endpoint-agreement refusal is deferred only while a provisional convergence plan carries a published boundary-clearance requirement. Production rendering settles that grant and immediately observes a strict plan with clearance publication disabled, so the same guard is active before anything can render. No committed corpus fixture reaches the provisional shortfall; the controlled regression lowers the placement floor to exercise both arms. |
| 5351 | `if ctx.validate_final_route_frames and not clearance_pending:` | `->L-5245` |  |
| 5365 | `if not plan.owns_geometry:` | `->L5366` |  |
| 5367 | `if plan.system_id in execution.query.clearance_requirement_system_ids:` | `->L5368` |  |
| 5376 | `if trunk_route is None or not _route_covers_trunk(trunk_route, plan.trunk_axis):` | `->L5377` |  |
| 5386 | `if route is None:` | `->L5387` |  |
| 5392 | `if (` | `->L5397` | **candidate-dead** -- Pre-existing gap, re-keyed from #13 by the gates the restored conflict conditions inserted earlier in the file (unchanged [0,0] split). Never executed rather than half-taken: its enclosing `if continuation.covered_by_member_id is not None:` never takes the None arm on any of the 18 fixtures that validate a planned convergence, so an uncovered outgoing continuation does not exist in this corpus. What would settle it is a topology whose continuation no endpoint owner's route spans, which _reconcile_continuation_ownership would then leave uncovered. |
| 5404 | `if any(` | `->L5408` |  |
| 5428 | `if (` | `->L5437` |  |
| 5443 | `if (` | `->L5454` |  |
| 5470 | `if route is None or membership is None:` | `->L5471` |  |
| 5476 | `if any(` | `->L5480` |  |
