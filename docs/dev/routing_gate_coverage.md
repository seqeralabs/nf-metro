...
**580/947** gates fully exercised (both/all arms hit by some fixture); **367** gates have at least one un-exercised arm.
...
| 1600 | `if lo_y < s.bbox_y + s.bbox_h and s.bbox_y < hi_y:` | `->L1601` |
| 1603 | `if blocker is None:` | `->L1605` |
| 1610 | `if left_ok and (not right_ok or abs(left_x - x) <= abs(right_x - x)):` | `->L1611`, `->L1612` |
| 1612 | `elif right_ok:` | `->L1613`, `->L1615` |
| 1637 | `if s.bbox_w <= 0 or s.grid_row == src_row:` | `->L1639` |
| 1639 | `if lo <= s.grid_col <= hi:` | `->L1636`, `->L1640` |
| 1665 | `if s.bbox_w <= 0:` | `->L1666` |

## `offsets.py`

176/256 gates fully exercised.
...
| 329 | `for e in graph.edges_to(cur):` | `->L324` |
| 330 | `if e.line_id != lid:` | `->L331` |
| 333 | `if port and port.is_entry:` | `->L335` |
| 336 | `if src and not src.is_port and src.section_id == sec_id:` | `->L329`, `->L337` |
