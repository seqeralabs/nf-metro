...
| 2333 | `if planned_transition is not None:` | `->L2334` |  |
| 2554 | `if query is None:` | `->L2555` | **defensive** -- Production routing runs after layout publishes FanPlanExecution. The None arm preserves the ordinary dispatcher for direct or incomplete routing contexts. |
| 2579 | `if ctx.station_offsets:` | `->L2581` | **defensive** -- _run_pass_c_guards always populates ctx.station_offsets before route_edges, and RAILS mode returns earlier; the empty-offsets arm never fires. Reclassified candidate-dead -> defensive (#762): corpus instrumentation across 128 fixtures confirms the un-exercised arm is never taken. |
| 2686 | `if query is None:` | `->L2687` |  |
| 2690 | `if binding is None:` | `->L2691` |  |
| 2695 | `plan.disposition is not FanPlanDisposition.PLANNED` | `->L2701` |  |
| 2696 | `or emission.emitter is not FanRouteEmitter.BOTTOM_EXIT_RIGHT_LANDINGS` | `->L2701` |  |
| 2697 | `or plan.fork_station_id != edge.source` | `->L2701` | **defensive** -- FanPlan membership validation requires every emission source to equal fork_station_id. This operand catches a frozen-plan/source mismatch before materialisation. |
| 2698 | `or target_port is None` | `->L2701` | **defensive** -- The planner creates this emitter only for a resolved landing port. A missing target port therefore indicates graph or plan drift after publication. |
| 2699 | `or target_port.side is not PortSide.RIGHT` | `->L2701` | **defensive** -- BOTTOM_EXIT_RIGHT_LANDINGS is selected only when every target is a RIGHT entry. The non-RIGHT arm is a defensive check that the frozen target still satisfies that contract. |
| 2710 | `if port is None or port.side is not PortSide.RIGHT or section is None:` | `->L2711` | **defensive** -- The planned emitter is created only from existing RIGHT landing ports with owning sections. This guard rejects stale or corrupted port references during materialisation. |
| 2714 | `if section not in landing_sections:` | `->L2705` | **defensive** -- The planner admits this emitter only when branches land in distinct sections, so the duplicate-section arm cannot occur for a valid frozen emission. It remains a materialisation-time drift check. |
| 2716 | `if len(landing_sections) != len(plan.branches):` | `->L2717` | **defensive** -- The planner emits this route family only when every branch lands in one distinct RIGHT-entry section. A count mismatch can arise only if the frozen emission and plan have drifted apart. |
| 2719 | `if plan.entry_runway is None:` | `->L2720` | **defensive** -- Every PLANNED FanPlan is validated with both runways before publication. The None arm is a runtime drift check for an incomplete plan object. |
| 2723 | `if not source_lines or edge.line_id not in source_lines:` | `->L2724` | **defensive** -- The planned emitter requires a non-empty offset_line_order whose line set exactly matches its emissions. The missing-line arm protects routing from a malformed frozen plan. |
| 2750 | `if route is None:` | `->L2751` |  |
| 2786 | `if tgt_port is None or tgt_port.side != PortSide.LEFT:` | `->L2787` |  |
| 2789 | `if ep_col is None or ep_row is None:` | `->L2790` |  |
| 2792 | `if corner_x is None:` | `->L2793` |  |
| 2915 | `if _would_route_around_section_below(other, ctx):` | `->L2916` | **defensive** -- Competing-sibling detection in _has_around_section_sibling, routed through the dispatch table (#813). A non-bypass sibling feeding the same merge junction sits in an adjacent column or a gap with no same-row intervening section, so it dispatches to an L-shape, not the around-below loop; corpus instrumentation across the fixtures confirms the arm is never taken. Defensive. |
| 3050 | `if trunk_shape.around_below:` | `->L3051` |  |
| 3059 | `if flank_xs:` | `->L3038` |  |
| 3150 | `if src_row is None or tgt_row is None or src_col is None or tgt_col is None:` | `->L3151` | **defensive** -- _is_row_level_bottom_row_climb's null guard; neither swept caller can pass a None coordinate. _bypass_geometry asserts src_col/tgt_col non-None and only calls it under cross_row, and cross_row is True only when both rows are set: merge_trunk_force_cross_row returns False unless src_row is not None and tgt_row == src_row, and the other disjunct requires both rows non-None. _fan_bypass_band passes non-None src_col/src_row and a checked tgt_col, and any placed target entry-port section resolves a non-negative grid_row. No fixture in the render corpus reaches the None arm. |
| 3304 | `and corridor.bypass_band_y >= base_y - COORD_TOLERANCE` | `->L3314` | **needs-review** -- Safety fallback: the shared bypass band is the deepest sibling's bypass_bottom_y, so a branch's own base_y never exceeds it; the arm that would decline a shallower shared band (guarding a predicate mismatch) is unexercised in the corpus. |
| 3427 | `if trunk_v_up_pull_away:` | `->L3436` | **needs-review** -- Reachable only via a defective render (merge-junction tangle); see #721. |
| 3466 | `this_xmin - gap_left >= SECTION_ROUTE_CLEARANCE` | `->L3467`, `->L3486` |  |
| 3467 | `and gap_right - around_xmax >= SECTION_ROUTE_CLEARANCE` | `->L3469`, `->L3486` |  |
| 3517 | `if src_sec is not None and src_sec.bbox_w > 0:` | `->L3567` | **defensive** -- A bypass source section always resolves and has bbox_w>0 (the parser never emits an empty section); the None/zero-width arm never fires. |
| 3699 | `if facts.src_col is None or facts.tgt_col is None:` | `->L3700` |  |
| 3704 | `if shape.around_below:` | `->L3705` |  |
| 3709 | `or _bypass_route_kind(facts) is not _BypassRoute.U_BYPASS` | `->L3711` |  |
| 3800 | `if ctx.exit_turns is None:` | `->L3801` |  |
| 3804 | `membership is None` | `->L3807` |  |
| 3809 | `if abs(start[0] - end[0]) > COORD_TOLERANCE:` | `->L3810` |  |
| 3873 | `membership is None` | `->L3876` |  |
| 3881 | `if abs(start[0] - end[0]) > COORD_TOLERANCE:` | `->L3882` | **defensive** -- _seat_bypass_descent reads the segment at BYPASS_DESCENT_RANK, which the U-bypass builder emits as the descent between the two gap corners and is vertical by construction. The non-vertical bail guards a route whose point order does not match the shape the caller just built. |
| 3920 | `if route is not None:` | `->L3922` |  |
| 3948 | `if route is None:` | `->L3949` | **defensive** -- Defensive in _declare_trunk: _route_inter_section only reaches the declaration after a matched rule (or the L-shape fall-through) produced a route, so the None guard never fires on the corpus; it mirrors _declare_channel's same guard for a handler that returns None. |
| 3976 | `if route is None:` | `->L3977` | **defensive** -- _declare_channel deliberately accepts RoutedPath \| None so declining builders can be ignored. Production callers currently pass concrete routes; the None no-op remains a defensive helper contract. |
| 4023 | `if route is None:` | `->L4024` | **defensive** -- _declare_placed_channels accepts RoutedPath \| None to match _declare_trunk and _declare_channel, but all three call sites have already established the route (an assert on the left-entry corridor, a RuntimeError when a planned fan emitter omits a member, and a locally built U-bypass), so the None no-op is a helper contract nothing exercises. |
| 4245 | `while cur is not None and cur not in seen:` | `->L4261` |  |
| 4251 | `if port.side == PortSide.LEFT:` | `->L4253` |  |
| 4254 | `if cur in graph.junctions:` | `->L4260` |  |
| 4341 | `if (` | `->L4347` |  |
| 4526 | `if geometry is None:` | `->L4527` |  |
| 4625 | `if abs(tgt.x - src.x) <= ctx.curve_radius:` | `->L4626` |  |
| 4638 | `if _h_segment_crosses_other_section(graph, src.x, final_x, src.y, exclude):` | `->L4639` | **defensive** -- Traverse-first clearance test in _top_entry_side_fan_traverse_clears (#1349). The below-side fan branch only reaches this gate when its junction also feeds an aligned straight drop; the corpus fan sits a clear row above its target, so the source-Y traverse never crosses another section and the fall-back-to-staircase arm is not exercised. Defensive. |
| 4690 | `if src_sec is None or tgt_sec is None:` | `->L4691` | **defensive** -- _flanked_cross_row_riser_seat_x's type-narrowing guard before it reads both sections' grid columns. The corpus reaches this seat only for exit-port sources whose section resolves, and every perpendicular-entry target owns a section, so no swept fixture hits the None arm; it stays as a defensive narrow. |
| 4825 | `if abs(tgt.x - src.x) > COORD_TOLERANCE or src.id not in ctx.graph.junctions:` | `->L4826` |  |
| 4860 | `if feeder is not None:` | `->L4865` |  |
| 5029 | `if direction is None:` | `->L5030` |  |
| 5058 | `if len(points) < 3:` | `->L5059` |  |
| 5112 | `if not section_ids:` | `->L5113` |  |
| 5192 | `if mid_y is None:` | `->L5209` |  |
| 5224 | `channel_y is None` | `->L5254` |  |
| 5226 | `and src_sec is not None` | `->L5254` | **defensive** -- Inter-row exit mid-Y clearance bump (the n>1 multi-line branch). When more than one line shares the exit the source section always resolves, so the `src_sec is None` short-circuit past the clearance bump is never taken. |
| 5227 | `and tgt_sec is not None` | `->L5254` | **defensive** -- Companion to src_sec in the same n>1 mid-Y clearance branch: the target section always resolves there, so the `tgt_sec is None` short-circuit past the bump is never taken. |
| 5256 | `if exit_side is not None:` | `->L5258` |  |
| 5258 | `elif abs(dx) > ctx.curve_radius:` | `->L5259`, `->L5261` |  |
| 5262 | `if src.id in ctx.graph.junctions:` | `->L5263`, `->L5275` | **needs-review** -- Reachable only via a defective render (LR->TB TOP-entry routing); see #720. |
| 5263 | `for je in ctx.graph.edges_to(src.id):` | `->L5264`, `->L5275` |  |
| 5265 | `if js.is_port:` | `->L5263`, `->L5266` |  |
| 5266 | `if abs(js.x - src.x) <= COORD_TOLERANCE:` | `->L5267`, `->L5269` |  |
| 5284 | `if exit_side is not None and not straight_drop:` | `->L5299` |  |
| 5337 | `if not straight_drop:` | `->L5343` | **needs-review** -- A top-entry fan branch fed straight from directly above the junction (straight_drop) keeps its lead-in column at the source X rather than the shared fan pivot; the corpus's fan branches all carry horizontal travel, so the straight-drop arm is unexercised. |
| 5368 | `if straight_drop:` | `->L5369` |  |
| 5450 | `if ctx.exit_turns is None:` | `->L5451` | **needs-review** -- _perp_entry_turn_is_planned answers False for the pre-plan context the exit-turn planner probes emitters with; ctx.exit_turns is bound only for emission. That context does reach these emitters -- the same guard in _seat_bypass_descent takes its None arm on packed_cell_right_exit_left_entry_wrap -- but no corpus probe reaches the TOP/BOTTOM-entry L-shape records this predicate feeds. |
| 5535 | `if src_sec is None or tgt_sec is None or src_sec.grid_row >= tgt_sec.grid_row:` | `->L5538` |  |
| 5705 | `if src_section and src_section.bbox_w > 0:` | `->L5706` |  |
| 5726 | `if tgt_sec is None:` | `->L5727` |  |
| 5797 | `if src_col is None:` | `->L5798` |  |
| 6041 | `if src_sec is not None:` | `->L6072` | **defensive** -- Defensive guard in the fanned entry-wrap source-clearance lift: a fan-out junction always resolves to its source section, so the None arm is unreachable in practice. |
| 6078 | `if tgt_col is not None:` | `->L6083` | **defensive** -- tgt_col from _resolve_section_col on a valid LEFT entry port is always non-None (sections get grid_col>=0 after auto-layout). |
| 6080 | `if shared_vx is not None:` | `->L6083` | **defensive** -- _fan_left_entry_descent_x returns None only when col_left<=0; col_left_edge for the target's own column is always >0. |
| 6206 | `if entry_side is PortSide.LEFT:` | `->L6209` |  |
| 6274 | `if entry_port is None:` | `->L6275` | **defensive** -- _route_around_section_below is always called with a non-None entry_port (both dispatch sites pass a checked station). |
| 6285 | `if trunk_src is None or trunk_src == edge.source:` | `->L6287` | **needs-review** -- Reachable only via a defective render (merge-junction tangle); see #721. |
| 6338 | `if col_left <= 0.0:` | `->L6339` | **defensive** -- col_left_edge's 0.0 default only fires for a column with no sections; the target's own column always holds its section. |
| 6375 | `if entry_port is None:` | `->L6376` | **defensive** -- Every caller of _corridor_is_viable passes a non-None entry_port. |
| 6382 | `if src_row is None or ep_row is None or src_col is None or ep_col is None:` | `->L6383` | **defensive** -- _resolve_section_colrow returns (None,None) only for a station with no section; valid entry ports always have a section with non-negative grid coords. |
| 6469 | `if fan is not None and corridor is not None and corridor.band_y is not None:` | `->L6473` | **needs-review** -- Corridor-feeder shared inter-row band gate: the corpus's corridor feeders belong to fans whose corridor carries a band_y, so the short-circuit past it (no corridor or bypass-only) is unexercised; it falls to the elif fan-is-not-None global-band branch. |
| 6473 | `elif fan is not None:` | `->L6477`, `->L6482` | **needs-review** -- Corridor-feeder fallback for a fan whose junction earned no corridor (its in-column gap below does not fit the bundle); the corpus's fans all earn a corridor, so this global-band branch is unexercised. |
| 6482 | `elif gap_bottom > gap_top:` | `->L6483`, `->L6487` | **needs-review** -- Reachable only via a defective render (inter-row corridor on a <78px gap); see #722. |
| 6492 | `if fan is None and gap_bottom > gap_top:` | `->L6493` | **needs-review** -- Reachable only via a defective render (inter-row corridor on a <78px gap); see #722. |
| 6506 | `if fan is not None and ep_col is not None:` | `->L6508` | **needs-review** -- Reachable only via a defective render (inter-row corridor on a <78px gap); see #722. |
| 6508 | `if vx is None:` | `->L6509` | **needs-review** -- Reachable only via a defective render (inter-row corridor on a <78px gap); see #722. |
| 6610 | `if ep_section and ep_section.bbox_w > 0:` | `->L6613` | **defensive** -- entry_port.section_id is always set and the section has bbox_w>0 after layout; the else arm never fires. |
| 6907 | `for claim in ctx.convergences.prior_channel_claims_for_edge(edge):` | `->L6908` |  |
| 6908 | `if claim.line_id != edge.line_id or claim.owner_source == edge.source:` | `->L6909`, `->L6910` |  |
| 6910 | `if not (corner_x - COORD_TOLERANCE <= claim.x <= gap_right + COORD_TOLERANCE):` | `->L6911`, `->L6912` |  |
| 6912 | `if min(hi, claim.y_hi) - max(lo, claim.y_lo) > COORD_TOLERANCE:` | `->L6907`, `->L6913` |  |
| 6986 | `if _leadout_self_meets_sibling_descent(ctx, edge, corner_x, f.sy, hy, gap_right):` | `->L6987` |  |
| 7118 | `if gap_bottom <= gap_top:` | `->L7119` | **defensive** -- The row-gap enforcer keeps stacked rows separated, so the inter-row band above a target row (row_bottom_edge(tgt_row-1) up to row_top_edge(tgt_row)) has positive height whenever a RIGHT entry is fed from a higher row; the degenerate-band arm guards an abutting-rows layout placement never produces. |
| 7122 | `if not _inter_row_band_fits(gap_top, gap_bottom):` | `->L7123` | **defensive** -- Band-fit test in _right_entry_gap_above_is_clear (#889), reached only when the cross-row RIGHT-entry drop-in is not clear (the exotic blocked-descent case above). The corpus always takes the drop-in, so neither arm of the gap-above fallback is exercised. Defensive. |
| 7277 | `if gap_bottom <= gap_top:` | `->L7278` |  |
| 7279 | `if not _inter_row_band_fits(gap_top, gap_bottom):` | `->L7280` |  |
| 7290 | `if f.v_segment_crosses_other_section(vx, gy, tgt.y):` | `->L7291` |  |
| 7385 | `if source is None or source.bbox_w <= 0:` | `->L7386` |  |
| 7398 | `if cellmate_left is None or other.bbox_x < cellmate_left:` | `->L7392` |  |
| 7438 | `if band0_bottom <= band0_top or not _inter_row_band_fits(band0_top, band0_bottom):` | `->L7439` |  |
| 7440 | `if band1_bottom <= band1_top or not _inter_row_band_fits(band1_top, band1_bottom):` | `->L7441` |  |
| 7442 | `if band1_top <= band0_bottom + COORD_TOLERANCE:` | `->L7443` |  |
| 7458 | `if f.v_segment_crosses_other_section(corner_x, band0_y, band1_y, exclude):` | `->L7459` |  |
| 7515 | `if geom is None:` | `->L7516` |  |
| 7518 | `if f.v_segment_crosses_other_section(geom.lead_x, src.y, geom.band0_y):` | `->L7519` |  |
| 7520 | `if f.h_segment_crosses_other_section(geom.lead_x, geom.corner_x, geom.band0_y):` | `->L7521` |  |
| 7522 | `if f.h_segment_crosses_other_section(geom.corner_x, geom.vx, geom.band1_y):` | `->L7523` |  |
...
| Line | Gate | Un-exercised arm(s) | Triage |
| ---: | --- | --- | --- |
| 157 | `if edge.target in inbound:` | `->L159` | **defensive** -- inbound is pre-built as {sid: set() for sid in graph.stations}, so every station id in the graph is a key. All edge endpoints are guaranteed to be in graph.stations after parsing (regular nodes via register_station, ports via add_port->add_station, junctions via add_station), so edge.target is always found in inbound and the False branch (54->56) is never taken. |
| 159 | `if edge.source in outbound:` | `->L156` | **defensive** -- outbound is pre-built from graph.stations keys identically to inbound. All edge sources are registered stations by construction (regular nodes, ports, junctions), so edge.source is always found in outbound and the False branch (56->53) is never taken. |
| 461 | `if ordered != expected:` | `->L462` | **defensive** -- The raise arm of _assert_sections_anchored_on_trunk, a backstop on the postcondition of _reindex_section_local: an independent section (no flat-frame neighbour) always lands on the contiguous top-anchored levels 0,step,...,(m-1)*step by construction, so the mismatch arm never fires across the corpus. It guards against a future change that stops re-anchoring such a section. |
| 509 | `while cur is not None and cur not in seen:` | `->L518` |  |
| 566 | `if neighbour is None:` | `->L567` |  |
| 587 | `if slot is None:` | `->L588` |  |
| 779 | `if reverse:` | `->L780` | **needs-review** -- Reachable only on a reconvergence section that is also a serpentine fold, where the fold corner crosses lines mid-curve and the return-row bundle sits one offset too low (#705). Exercise with a clean fixture once #705 is fixed. |
| 810 | `if section is None:` | `->L811` |  |
| 834 | `if not all(lid in feeder_off for lid in continuing):` | `->L835` |  |
| 842 | `if lid in new_off:` | `->L841` |  |
| 864 | `if section is not None:` | `->L862` |  |
| 866 | `if len(set(feeder_row.values())) < 2:` | `->L867` |  |
| 886 | `if entry_port is None:` | `->L887` |  |
| 929 | `if seam is None:` | `->L930` |  |
| 951 | `if not all(lid in feeder_off for lid in primary_lines):` | `->L952` |  |
| 1000 | `if new_order is None:` | `->L1003` |  |
| 1045 | `if len(feeder_ids) != 1:` | `->L1046` |  |
| 1050 | `if feeder is None or not lanes_run_along_y(feeder.direction):` | `->L1051` |  |
| 1052 | `if feeder.entry_ports:` | `->L1053` |  |
| 1054 | `if _section_exit_fanout_junction(ctx, feeder) is not None:` | `->L1055` |  |
| 1062 | `if consumers != {section.id}:` | `->L1063` |  |
| 1072 | `for pid in section.entry_ports:` | `->L1078` |  |
| 1074 | `if port is None or port.side not in perp_sides:` | `->L1075` |  |
| 1076 | `if bundle <= set(graph.station_lines(pid)):` | `->L1072` |  |
| 1156 | `if port_id is None:` | `->L1157` |  |
| 1228 | `if swap_line is None:` | `->L1229` | **defensive** -- The desired peel order is a permutation of the offsets already assigned to the complete fan-out exit bundle, so each displaced line's target offset is held by another line. A missing swap partner requires an incomplete or duplicate offset map; recompacted_fanout_exit exercises the successful swap body. |
| 1268 | `if cur in seen:` | `->L1269` | **needs-review** -- In the exit-only-line reorder / same-Y offset propagation; reachable only via a multi-line LR/RL section with a perpendicular (TOP/BOTTOM) exit port, which the engine lays out as a station-as-elbow + collinear overlay PhaseInvariantError (#706). Exercise with a clean fixture once #706 is fixed. |
| 1278 | `if not src.is_port and src.section_id == sec_id:` | `->L1271` |  |
| 1364 | `if not target_id:` | `->L1365` | **defensive** -- An exit-only line at station sid is identified by being present in station_lines(sid) but absent from ctx.inbound[sid]. station_lines() returns lines appearing in any edge whose source or target is sid. For a line to be exit-only at sid, it must appear in at least one outbound edge from sid; otherwise it would not appear in station_lines(sid) at all. If an outbound edge exists, outbound_target[(sid, lid)] is always set, so target_id is never None when execution reaches L422. Reclassified candidate-dead -> defensive (#762): corpus instrumentation across 128 fixtures confirms the un-exercised arm is never taken. |
| 1367 | `if not target_st:` | `->L1368` | **defensive** -- After parse and layout, every edge target ID in a MetroGraph maps to a station in graph.stations. The guard protects against a structurally malformed graph where an edge references a nonexistent station ID, which cannot occur with valid .mmd input processed through the parser and _resolve_sections(). |
| 1385 | `if swap_lid is None:` | `->L1386` | **defensive** -- In _reorder_one_exit_line, swap_lid is None only if the loop over `lines` exhausts without a match, but desired_off (min or max of all_offs) is always held by another line, so the early-return guard never fires (0/128). Defensive. Reclassified defensive (#762). |
| 1442 | `if abs(nbr_cur - new_off) < _OFFSET_EQ_TOLERANCE:` | `->L1443` | **needs-review** -- In the exit-only-line reorder / same-Y offset propagation; reachable only via a multi-line LR/RL section with a perpendicular (TOP/BOTTOM) exit port, which the engine lays out as a station-as-elbow + collinear overlay PhaseInvariantError (#706). Exercise with a clean fixture once #706 is fixed. |
| 1464 | `if max_steps <= 0:` | `->L1465` | **defensive** -- max_steps = len(graph.stations) * len(graph.lines). Each BFS iteration dequeues one (station, line) pair and marks it visited; visited pairs are skipped on re-encounter. The number of distinct (station, line) pairs is bounded by \|stations\| * \|lines\|, so the BFS can perform at most that many productive steps before the queue empties. The `while queue and max_steps > 0` condition therefore always exits via the queue-empty branch; max_steps reaches zero only if more productive steps occur than unique pairs, which is impossible in a finite graph. |
| 1492 | `if lid not in seen:` | `->L1489` | **defensive** -- The iterator at L554 is `sorted(set(sec_entry_lines), ...)`. The outer `set()` deduplicates the list before iteration, so each `lid` appears exactly once and `seen` never contains it when processed. The false arm (lid already in seen) is logically unreachable — the dedup happens at set construction, making the `seen` guard dead code. Reclassified candidate-dead -> defensive (#762): corpus instrumentation across 128 fixtures confirms the un-exercised arm is never taken. |
| 1619 | `for _ in range(len(graph.stations) + 1):` | `->L1636` |  |
| 1623 | `if edge is None:` | `->L1624` |  |
| 1626 | `if nxt in seen:` | `->L1627` |  |
| 1710 | `if not src_st.is_port:` | `->L1708` |  |
| 1714 | `if internal_offs:` | `->L1702` | **defensive** -- Exit ports are only created when inter-section edges exist: resolve.py `_create_port_stations` iterates `topology.exit_groups`, which parser/route_topology.py builds from its `exit_members` map, populated one entry per resolved inter-section edge. Every TB LEFT/RIGHT exit port therefore has at least one edge from an internal station, so `internal_offs` is never empty when the loop is entered. The false arm is a defensive guard against an impossible empty-feeder state. |
| 1744 | `if not src_st.is_port:` | `->L1742` |  |
| 1875 | `if anchor_feeders:` | `->L1878` |  |
| 1893 | `if port_off is not None:` | `->L1891` | **defensive** -- _compute_base_offsets runs before junction propagation and creates an offset for every station-line pair. A divergence exit port and its junction carry the same connector lines, so this lookup always succeeds; the missing-offset arm protects an incomplete synthetic context. |
| 1978 | `if not ctx.bundle_re_slots_whole[(junction_id, exit_port_id)]:` | `->L1979` |  |
| 1991 | `if len(drops) == len(lines):` | `->L1975` |  |
| 2088 | `if port_st is None:` | `->L2089` |  |
| 2163 | `if not src.is_port:` | `->L2164` |  |
| 2320 | `if not src.is_port:` | `->L2321` |  |
| 2347 | `if paired_off is not None:` | `->L2345` | **defensive** -- _compute_base_offsets sets ctx.offsets[(sid, lid)] for every station-line pair before _propagate_lr_rl_exit_to_entry runs. Exit ports are stations. The for-loop at L791 iterates graph.station_lines(port_id) which equals graph.station_lines(exit_port_id) (enforced by the exit_lines != entry_lines guard at L788). For each lid, ctx.offsets.get((exit_port_id, lid)) is therefore never None. The loop-back arc (793,791) for the paired_off-is-None case is structurally unreachable. |
| 2353 | `if not tgt_st.is_port:` | `->L2351` |  |
| 2405 | `if set(inherited) != set(graph.station_lines(port_id)):` | `->L2406` | **defensive** -- _inherit_level_convergence_entry_offsets skips a convergence entry port when the lines it inherits from its immediate feeders do not cover every line the port carries. A convergence entry's lines are, by construction, exactly the union of its inbound edges' lines, and each inbound edge supplies that line's feeder offset, so the inherited set always equals the port's line set and the mismatch arm is never taken. Defensive against a port line with no inbound edge (a malformed graph). |
| 2412 | `if _bundle_reslot_collides(ctx, port_id, port_obj.section_id, inherited):` | `->L2413` | **needs-review** -- The level-convergence inherit is refused when carrying the feeders' order along the bundle would seat a bundle line on the slot a section-local line already holds. No fixture in the sweep corpus (examples plus the top-level tests/fixtures) presents that collision; tests/fixtures/hash_seed_determinism/seed_77.mmd does, but the sweep does not descend into the nested seed directories, so closing the arm needs a corpus topology of its own. |
| 2444 | `if entry.side not in (PortSide.LEFT, PortSide.RIGHT):` | `->L2445` | **defensive** -- In _align_flat_tb_exit_to_entry: scopes the flat-seam exit/entry reconciliation to horizontal (LEFT/RIGHT) entry ports, whose per-line offsets are Y like the TB exit's. A TOP/BOTTOM entry stores X-offsets, so copying across would be a category error. No corpus fixture routes a TB LEFT/RIGHT exit directly into a TOP/BOTTOM entry at the same Y, so the skip arm is never taken; defensive against that cross-axis seam. |
| 2452 | `if len(shared) < 2:` | `->L2453` | **defensive** -- In _align_flat_tb_exit_to_entry: scopes the flat-seam reconciliation to multi-line bundles, where a constant per-line offset delta means the lines share an order and only the bundle anchoring differs (the auto-folded serpentine case in #1198). A single shared line carries no bundle order to preserve and is left to the single-line corridor passes; no corpus fixture reaches the pass with fewer than two shared lines, so the skip arm is never taken. |
| 2541 | `if edge.line_id not in lines:` | `->L2542` |  |
| 2598 | `for edge in graph.edges_from(port_id):` | `->L2607` |  |
| 2602 | `if tgt.is_port:` | `->L2603` |  |
| 2607 | `if target_id is None:` | `->L2608` |  |
| 2692 | `if not sec_stations:` | `->L2693` | **defensive** -- The parser calls _remove_empty_sections() (resolve.py:26) which deletes any section whose station_ids list is empty before port creation. Ports are only added to station_ids during _create_port_stations() (resolve.py:527), which runs AFTER _remove_empty_sections(). Therefore every section in graph.sections at layout time has at least one non-port station; sec_stations is never empty. |
| 2737 | `if station.section_id is not None:` | `->L2729` |  |
| 2746 | `if port.section_id is None:` | `->L2747` |  |
| 2759 | `if section is None:` | `->L2760` |  |
| 2908 | `if tgt_port.side not in (PortSide.LEFT, PortSide.RIGHT):` | `->L2909` |  |
| 2910 | `if tgt_port.section_id in ctx.tb_sections:` | `->L2911` |  |
| 2915 | `if not _stores_reflected(ctx, tgt_port.section_id) and reversed_seam:` | `->L2916` |  |
| 2918 | `if off is not None:` | `->L2904` |  |
| 2938 | `if not any(` | `->L2941` | **defensive** -- The L920 check returns None when offsets are EXACTLY equal to expected (exact float ==). The L926 check returns None when differences are less than _OFFSET_EQ_TOLERANCE (0.001). Since all offsets are integer multiples of OFFSET_STEP (3.0px), any non-zero difference is exactly 3.0 or more, which is >> 0.001. So L920 catches exact matches and L926 can never catch a case L920 missed; the near-equal window (0,0.001) is unreachable with 3.0px discrete steps. |
| 2956 | `if len(seed_lines) < 2:` | `->L2957` |  |
| 2962 | `if not fixed_here or not movable:` | `->L2963` |  |
| 2981 | `for lid in above:` | `->L2982` |  |
| 2985 | `if not any(` | `->L2988` |  |
| 3077 | `if not changed_lids:` | `->L3078` |  |
| 3096 | `if pending is None:` | `->L3097` |  |
| 3225 | `if max_steps <= 0:` | `->L3226` | **defensive** -- max_steps = n_sec_stations * len(graph.lines). The BFS visits each (station_id, line_id) pair at most once (the visited set at L1017 prevents revisits). The total distinct pairs is bounded by n_sec_stations * len(graph.lines) = max_steps. Since max_steps decrements by 1 per loop iteration and the loop exits when the queue is empty (naturally after at most max_steps unique visits), the condition max_steps <= 0 is never True; the while-loop guard already prevents the body from executing when the queue is empty. |
| 3239 | `if sec_a is None and id_a in graph.ports:` | `->L3240` | **defensive** -- The true arm requires sec_a is None AND id_a in graph.ports simultaneously. Junction stations (section_id=None) are added only via graph.add_junction() which calls graph.add_station() but NOT graph.add_port(). graph.add_port() (model.py:483) requires a Port object with a non-None section_id (Port.section_id is typed str, not Optional[str], per model.py:218). Therefore no station with section_id=None can ever be in graph.ports; the conjunction sec_a is None AND id_a in graph.ports is unsatisfiable. Reclassified candidate-dead -> defensive (#762): corpus instrumentation across 128 fixtures confirms the un-exercised arm is never taken. |
| 3241 | `if sec_b is None and id_b in graph.ports:` | `->L3242` | **defensive** -- Same argument as L1084: id_b in graph.ports requires the station to have been registered via graph.add_port(), which always supplies a non-None section_id for the created station. A station with section_id=None (sec_b is None) is a junction created by add_junction(), which never calls add_port(). The precondition sec_b is None AND id_b in graph.ports is therefore unsatisfiable. Reclassified candidate-dead -> defensive (#762): corpus instrumentation across 128 fixtures confirms the un-exercised arm is never taken. |
| 3312 | `if (tgt_id, lid) not in ctx.offsets:` | `->L3313` | **defensive** -- The True branch (missing entry-port offset -> ok=False) is unreachable because phase 7 (_compute_entry_port_offsets) runs before this function (phase 7a) and assigns an offset for every (entry_port_id, line_id) pair in the graph. By the time _align_junction_to_entry_port runs, ctx.offsets is populated for all entry ports, so the guard never fires. Reclassified candidate-dead -> defensive (#762): corpus instrumentation across 128 fixtures confirms the un-exercised arm is never taken. |
| 3332 | `if src_port and not src_port.is_entry:` | `->L3339` | **defensive** -- Reaches the realignment body of _align_junction_to_entry_port only via a TB-reversed two-line bundle fanning into a junction (the #704 boundary fix is in, so this body is now provably reachable). The junction_entry_reversed_fold fixture now ships that topology; with the bundle order carried concentrically through the reversal corners the junction offsets already match their entry ports, so the realignment body short-circuits before its offset-writing arms and they stay un-exercised corpus-wide. Defensive. Its else arm (->1203) is additionally defensive: the sole feeder of a >=2-line fan-out junction is its exit port, a non-entry port. |
| 3335 | `elif feeding_exit != edge.source:` | `->L3336` | **defensive** -- This arm (single_exit=False due to multiple distinct exit ports feeding one junction) requires one junction to receive edges from two different exit ports. But _rewrite_edges_with_junctions in resolve.py creates exactly one junction per exit port fan-out, wired exclusively from that exit port's edges. A junction always has a single upstream exit port; the elif is structurally unreachable. Reclassified candidate-dead -> defensive (#762): corpus instrumentation across 128 fixtures confirms the un-exercised arm is never taken. |
| 3344 | `if single_exit and feeding_exit is not None:` | `->L3283` | **defensive** -- Reaches the realignment body of _align_junction_to_entry_port only via a TB-reversed two-line bundle fanning into a junction (the #704 boundary fix is in, so this body is now provably reachable). The junction_entry_reversed_fold fixture now ships that topology; with the bundle order carried concentrically through the reversal corners the junction offsets already match their entry ports, so the realignment body short-circuits before its offset-writing arms and they stay un-exercised corpus-wide. Defensive. Its False arm (->1147) is additionally defensive: a >=2-line junction reaching this body is always a single-exit-port-fed fan-out junction (merge junctions carry one line and are skipped by len(j_lines)<2). |
