...
**2294/3240** gates fully exercised (both/all arms hit by some fixture); **946** gates have at least one un-exercised arm.
...
| 3229 | `if entry is None or not entry.is_entry:` | `->L3230` | **needs-review** -- _fan_entry_ranks_first skips a divergence-junction branch whose target is not an entry port. Every corpus fan hands its shared branch straight to an entry port; a junction that feeds a merge junction first (the case fanout_divergence_peel_order resolves through _fan_branch_entry_port) would take this arm, and no swept fixture has a two-line branch of that shape. |
| 3231 | `if _stores_reflected(ctx, entry.section_id) != _stores_reflected(` | `->L3234` | **needs-review** -- _fan_entry_ranks_first declines when the shared entry's section stores its lanes reflected and the source's does not, or the reverse, since the raw offsets then rank the pair in opposite frames. Reaching it needs a gap-compaction collision in front of a fan whose shared branch enters a reverse-flow horizontal section; no swept fixture combines the two. |
