...
**2282/3223** gates fully exercised (both/all arms hit by some fixture); **941** gates have at least one un-exercised arm.
...
| 5278 | `if not straight_drop:` | `->L5282` | **needs-review** -- A top-entry fan branch fed straight from directly above the junction (straight_drop) keeps its lead-in column at the source X rather than the shared fan pivot; the corpus's fan branches all carry horizontal travel, so the straight-drop arm is unexercised. |
| 5307 | `if straight_drop:` | `->L5308` |  |
| 5389 | `if ctx.exit_turns is None:` | `->L5390` | **needs-review** -- _perp_entry_turn_is_planned answers False for the pre-plan context the exit-turn planner probes emitters with; ctx.exit_turns is bound only for emission. That context does reach these emitters -- the same guard in _seat_bypass_descent takes its None arm on packed_cell_right_exit_left_entry_wrap -- but no corpus probe reaches the TOP/BOTTOM-entry L-shape records this predicate feeds. |
| 5474 | `if src_sec is None or tgt_sec is None or src_sec.grid_row >= tgt_sec.grid_row:` | `->L5477` |  |
| 5644 | `if src_section and src_section.bbox_w > 0:` | `->L5645` |  |
| 5665 | `if tgt_sec is None:` | `->L5666` |  |
| 5736 | `if src_col is None:` | `->L5737` |  |
| 5980 | `if src_sec is not None:` | `->L6011` | **defensive** -- Defensive guard in the fanned entry-wrap source-clearance lift: a fan-out junction always resolves to its source section, so the None arm is unreachable in practice. |
| 6017 | `if tgt_col is not None:` | `->L6022` | **defensive** -- tgt_col from _resolve_section_col on a valid LEFT entry port is always non-None (sections get grid_col>=0 after auto-layout). |
| 6019 | `if shared_vx is not None:` | `->L6022` | **defensive** -- _fan_left_entry_descent_x returns None only when col_left<=0; col_left_edge for the target's own column is always >0. |
| 6145 | `if entry_side is PortSide.LEFT:` | `->L6148` |  |
| 6213 | `if entry_port is None:` | `->L6214` | **defensive** -- _route_around_section_below is always called with a non-None entry_port (both dispatch sites pass a checked station). |
| 6224 | `if trunk_src is None or trunk_src == edge.source:` | `->L6226` | **needs-review** -- Reachable only via a defective render (merge-junction tangle); see #721. |
| 6277 | `if col_left <= 0.0:` | `->L6278` | **defensive** -- col_left_edge's 0.0 default only fires for a column with no sections; the target's own column always holds its section. |
| 6314 | `if entry_port is None:` | `->L6315` | **defensive** -- Every caller of _corridor_is_viable passes a non-None entry_port. |
| 6321 | `if src_row is None or ep_row is None or src_col is None or ep_col is None:` | `->L6322` | **defensive** -- _resolve_section_colrow returns (None,None) only for a station with no section; valid entry ports always have a section with non-negative grid coords. |
| 6408 | `if fan is not None and corridor is not None and corridor.band_y is not None:` | `->L6412` | **needs-review** -- Corridor-feeder shared inter-row band gate: the corpus's corridor feeders belong to fans whose corridor carries a band_y, so the short-circuit past it (no corridor or bypass-only) is unexercised; it falls to the elif fan-is-not-None global-band branch. |
| 6412 | `elif fan is not None:` | `->L6416`, `->L6421` | **needs-review** -- Corridor-feeder fallback for a fan whose junction earned no corridor (its in-column gap below does not fit the bundle); the corpus's fans all earn a corridor, so this global-band branch is unexercised. |
| 6421 | `elif gap_bottom > gap_top:` | `->L6422`, `->L6426` | **needs-review** -- Reachable only via a defective render (inter-row corridor on a <78px gap); see #722. |
| 6431 | `if fan is None and gap_bottom > gap_top:` | `->L6432` | **needs-review** -- Reachable only via a defective render (inter-row corridor on a <78px gap); see #722. |
| 6445 | `if fan is not None and ep_col is not None:` | `->L6447` | **needs-review** -- Reachable only via a defective render (inter-row corridor on a <78px gap); see #722. |
| 6447 | `if vx is None:` | `->L6448` | **needs-review** -- Reachable only via a defective render (inter-row corridor on a <78px gap); see #722. |
| 6549 | `if ep_section and ep_section.bbox_w > 0:` | `->L6552` | **defensive** -- entry_port.section_id is always set and the section has bbox_w>0 after layout; the else arm never fires. |
| 6846 | `for claim in ctx.convergences.prior_channel_claims_for_edge(edge):` | `->L6847` |  |
| 6847 | `if claim.line_id != edge.line_id or claim.owner_source == edge.source:` | `->L6848`, `->L6849` |  |
| 6849 | `if not (corner_x - COORD_TOLERANCE <= claim.x <= gap_right + COORD_TOLERANCE):` | `->L6850`, `->L6851` |  |
| 6851 | `if min(hi, claim.y_hi) - max(lo, claim.y_lo) > COORD_TOLERANCE:` | `->L6846`, `->L6852` |  |
| 6925 | `if _leadout_self_meets_sibling_descent(ctx, edge, corner_x, f.sy, hy, gap_right):` | `->L6926` |  |
| 7057 | `if gap_bottom <= gap_top:` | `->L7058` | **defensive** -- The row-gap enforcer keeps stacked rows separated, so the inter-row band above a target row (row_bottom_edge(tgt_row-1) up to row_top_edge(tgt_row)) has positive height whenever a RIGHT entry is fed from a higher row; the degenerate-band arm guards an abutting-rows layout placement never produces. |
| 7061 | `if not _inter_row_band_fits(gap_top, gap_bottom):` | `->L7062` | **defensive** -- Band-fit test in _right_entry_gap_above_is_clear (#889), reached only when the cross-row RIGHT-entry drop-in is not clear (the exotic blocked-descent case above). The corpus always takes the drop-in, so neither arm of the gap-above fallback is exercised. Defensive. |
| 7216 | `if gap_bottom <= gap_top:` | `->L7217` |  |
| 7218 | `if not _inter_row_band_fits(gap_top, gap_bottom):` | `->L7219` |  |
| 7229 | `if f.v_segment_crosses_other_section(vx, gy, tgt.y):` | `->L7230` |  |
| 7324 | `if source is None or source.bbox_w <= 0:` | `->L7325` |  |
| 7337 | `if cellmate_left is None or other.bbox_x < cellmate_left:` | `->L7331` |  |
| 7377 | `if band0_bottom <= band0_top or not _inter_row_band_fits(band0_top, band0_bottom):` | `->L7378` |  |
| 7379 | `if band1_bottom <= band1_top or not _inter_row_band_fits(band1_top, band1_bottom):` | `->L7380` |  |
| 7381 | `if band1_top <= band0_bottom + COORD_TOLERANCE:` | `->L7382` |  |
| 7397 | `if f.v_segment_crosses_other_section(corner_x, band0_y, band1_y, exclude):` | `->L7398` |  |
| 7454 | `if geom is None:` | `->L7455` |  |
| 7457 | `if f.v_segment_crosses_other_section(geom.lead_x, src.y, geom.band0_y):` | `->L7458` |  |
| 7459 | `if f.h_segment_crosses_other_section(geom.lead_x, geom.corner_x, geom.band0_y):` | `->L7460` |  |
| 7461 | `if f.h_segment_crosses_other_section(geom.corner_x, geom.vx, geom.band1_y):` | `->L7462` |  |
