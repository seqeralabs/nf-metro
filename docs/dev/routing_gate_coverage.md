...
**2289/3229** gates fully exercised (both/all arms hit by some fixture); **940** gates have at least one un-exercised arm.
...
| 2004 | `if trunk_ys[-1] - trunk_ys[0] <= COORD_TOLERANCE:` | `->L2005` | **needs-review** -- Equal-depth guard on iter_port_peeloff_bundles' shared bundle grouping: the sole corpus fixture that ever produced two peel-off members at coincident trunk depth (riboseq_band_hop_zero_length_seam.mmd) now diverges before reaching this comparison, per #1772's trunk-fork fan reordering (see tests/test_band_hop_zero_length_seam_1808.py). No fixture currently reaches the same-Y reject arm. |
