...
The plan also names the station or port that supplies its absolute centreline.
Stages 4.9 and 6.17 read that fixed source after structural anchors settle. They
do not search the live grid or port topology for a different source, and the
...
- **Stage 6.17**: Re-materialise complete semantic fan frames on their settled
  ...
  branch is the topology's unique continuation. Unsupported symmetric diamonds
  then receive the legacy half-pitch compaction.
...
On the normal dispatcher path, routing then computes per-line station offsets,
...
before dispatch, `_route_edges` applies planned fan carriers and builds the
...
The Pass C tail (Stages 6.1 to 6.18) looks excessive at first glance.
