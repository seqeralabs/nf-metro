...
| 5   | `TB_BOTTOM_EXIT_AROUND_STACK`         | `TB bottom exit around stack`           | A TB/BT trailing-exit run, or a junction's sole diverting branch continuing one down its column, crosses sections stacked in a grid row between source and target. | `_route_around_stack` diverts through a clear gap.                                                           |
...
| 35  | `RIGHT_ENTRY_CROSS_ROW_WRAP`          | `RIGHT entry cross-row wrap`            | A source above and right of a RIGHT entry, merged or not, travels left unobstructed.                                                                               | `_route_right_entry_cross_row` uses the target-side band or around-below route.                              |
