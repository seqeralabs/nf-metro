...
The dev dependency group adds `pytest`, `ruff`, `mypy`.
...
uv run pytest tests/test_topology_validation.py
...
1. Run `uv run pytest` and `uv run ruff format --check`.
...
3. Run `uv run pytest tests/test_topology_validation.py` to confirm it passes (or fails, if you are pinning a known issue).
...
nf-metro render examples/rnaseq_sections.mmd -o /tmp/out.png
open /tmp/out.png
```

In a dev checkout, run the CLI as `uv run nf-metro …`, or `uv tool install --editable .` once for a bare `nf-metro` on PATH that tracks your working tree.

Batch-render the whole topology library with `uv run python scripts/render_topologies.py` (output in `/tmp/nf_metro_topology_renders/`).
