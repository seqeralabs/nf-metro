...
Dependencies: `av`, `click`, `drawsvg`, `lark`, `networkx`, `pillow`, `pyyaml`, `resvg-py`, `rich`, `rich-click`.
...
The terminal images in the [CLI reference](/nf-metro/cli/) and the guide are real output, captured with [rich-codex](https://ewels.github.io/rich-codex/).
Each one is declared where it is used, as a markdown image whose alt-text is the command that produced it:
...
A `RICH-CODEX` comment above an image sets that image's options (`<!-- ... -->` in `.md`, `{/* ... */}` in `.mdx`).
Any command that takes a repo path needs `working_dir: .` there, because a markdown-defined command otherwise runs in the markdown file's own directory.
`.rich-codex.yml` holds only the settings shared by every image.

`.github/workflows/rich-codex.yml` regenerates and commits them whenever `src/nf_metro/` changes on `main`, so a changed message or station count never leaves a stale image behind.
Regenerate them by hand if you want the new image in the same PR:

```bash frame="terminal"
pip install rich-codex
rich-codex --search-include 'docs/**/*.md*'
```

Each command cleans up after itself, so only `docs/assets/img/*.svg` should change.
...
# Render straight to PNG for review
python -m nf_metro render examples/rnaseq_sections.mmd -o /tmp/out.png
open /tmp/out.png
```
