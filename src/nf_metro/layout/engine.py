...
def _compute_section_layout(
    ...
    # Phase 13n: Generalised column-clearance pass.  Where a station's
    # name label (above or below its marker) sits in the same column as
    # an adjacent file-icon (with its under-icon caption) or another
    # station, ensure the rendered label / caption text doesn't crash
    # into the neighbour's marker bbox.  Phase 13m handled icon-icon
    # stacking; this pass also catches station-icon and station-station
    # cases in the same column.
    _pad_column_label_clearance(graph, section_y_padding)
...
def _pad_column_label_clearance(
    graph: MetroGraph,
    section_y_padding: float,
) -> None:
    """Ensure vertical clearance between adjacent elements in a column.

    Generalises ``_pad_stacked_captioned_file_icons`` (Phase 13m) to all
    label-bearing elements in a section column: file icons (with under-
    icon captions) and regular station markers (with above/below name
    labels).  Two examples that this catches:

    * gprofiler2's below-the-marker name label colliding with the top
      edge of the Network TSV file icon directly beneath it (DA
      section 3, with ``center_ports`` on).
    * The Zip-bundle station's above-the-marker name label colliding
      with the under-icon caption of the Shiny HTML file icon directly
      above it (DA section 5).

    For each LR/RL section, build effective top / bottom extents for
    every internal station -- including label or caption text -- group
    by rounded X column, and walk vertically-adjacent pairs.  When the
    upper element's lower extent plus ``ICON_STACK_LABEL_CLEARANCE``
    crosses the lower element's upper extent, shift the lower element
    (and its linear consumer chain inside the section) down by the
    deficit.  Cumulative shifts propagate to subsequent pairs in the
    column.

    Grows the section bbox downward as needed; lower-row sections are
    pushed to maintain ``section_y_gap``.
    """
    if not graph.sections:
        return
    ...
    def _recompute_label_placements() -> tuple[
        dict[tuple[str, str], float],
        dict[str, LabelPlacement],
    ]:
        # Snapshot section bboxes: ``place_labels`` mutates section
        # bbox_x / bbox_w to clamp labels horizontally, which would
        # corrupt port-on-boundary invariants and is purely an
        # exploratory call here.
        bbox_snapshot = {
            s.id: (s.bbox_x, s.bbox_y, s.bbox_w, s.bbox_h)
            for s in graph.sections.values()
        }
        try:
            offs = compute_station_offsets(graph)
            rts = route_edges(graph, station_offsets=offs)
        except Exception:  # noqa: BLE001 - routing failure surfaces elsewhere
            return {}, {}
        ic_obs = _compute_icon_obstacles_for_layout(graph)
        pls = place_labels(
            graph,
            station_offsets=offs,
            icon_obstacles=ic_obs,
            routes=rts,
        )
        for sid, (bx, by, bw, bh) in bbox_snapshot.items():
            ...
            sec.bbox_w = bw
            sec.bbox_h = bh
        return offs, {p.station_id: p for p in pls}

    station_offsets, placement_by_sid = _recompute_label_placements()
    if not placement_by_sid:
        return

    def _icon_offset_mid(st: Station) -> float:
        """Vertical shift of the icon center relative to the station Y.

        Mirrors ``render.svg`` which renders terminus icons at
        ``station.y + (min_off + max_off) / 2`` so multi-line stations
        with non-zero line offsets centre the icon on the line bundle.
        """
        lines = list(graph.station_lines(st.id))
        if not lines:
            return 0.0
        offs = [station_offsets.get((st.id, lid), 0.0) for lid in lines]
        return (min(offs) + max(offs)) / 2

    def _element_extents(st: Station) -> tuple[float, float]:
        """Top, bottom extent of a station including its label/caption."""
        if st.is_terminus:
            icon_cy = st.y + _icon_offset_mid(st)
            top = icon_cy - ICON_HALF_HEIGHT
            bot = icon_cy + ICON_HALF_HEIGHT
            names = st.terminus_names or []
            if any(bool(n) for n in names):
        ...
        if not st.label.strip():
            return top, bot
        lp = placement_by_sid.get(st.id)
        if lp is None:
            return top, bot
        # dominant-baseline=auto (above): baseline at lp.y, text above
        # dominant-baseline=hanging (below): top of text at lp.y, text below
        if lp.above:
    ...
    def _shift_chain_in_section(src: str, delta: float, internal_set: set[str]) -> None:
        cur = src
        src_lines = set(graph.station_lines(src))
        visited: set[str] = {src}
        while True:
    ...
    # Iterate placement + clearance until stable.  Re-running
    # ``place_labels`` after a shift can flip a label's side (the
    # alternation trial picks a different cost minimum once Ys move),
    # which then changes the required clearance.  In practice this
    # converges in 1-3 passes for real maps; cap at MAX_PASSES to keep
    # the run bounded on pathological inputs.
    MAX_PASSES = 5
    grew_sections_set: set[str] = set()
    for _ in range(MAX_PASSES):
        ...
        for section in graph.sections.values():
            ...
            for col_sids in cols.values():
                ...
                col_sids.sort(key=lambda s: graph.stations[s].y)
                for i in range(1, len(col_sids)):
                    upper_id = col_sids[i - 1]
                    lower_id = col_sids[i]
                    upper = graph.stations[upper_id]
                    lower = graph.stations[lower_id]
                    _, upper_bot = _element_extents(upper)
                    lower_top, _ = _element_extents(lower)
                    deficit = upper_bot + clearance - lower_top
                    if deficit <= 0.5:
                        continue
                    lower.y += deficit
                    _shift_chain_in_section(lower_id, deficit, internal_set)
                    any_shift = True
                    # Grow bbox if the lower element's extent now sits
                    # past the section's bottom padding line.
                    _, new_lower_bot = _element_extents(graph.stations[lower_id])
                    needed_bbox_bot = new_lower_bot + section_y_padding
                    cur_bbox_bot = section.bbox_y + section.bbox_h
                    if needed_bbox_bot > cur_bbox_bot:
                        section.bbox_h = needed_bbox_bot - section.bbox_y
                        grew_sections_set.add(section.id)

        if not any_shift:
            break

        # Recompute label placements with the new station Ys; a shift
        # can change the alternation trial-cost winner and therefore
        # the label side on previously-cleared stations.
        new_offsets, new_placements = _recompute_label_placements()
        if not new_placements:
            break
        station_offsets = new_offsets
        placement_by_sid = new_placements

    grew_sections = [
        graph.sections[sid] for sid in grew_sections_set if sid in graph.sections
    ]
    if grew_sections:
        _push_lower_rows_after_bbox_grow(graph, SECTION_Y_GAP)


def _compute_icon_obstacles_for_layout(
    graph: MetroGraph,
) -> list[tuple[float, float, float, float]]:
    """Build icon-bbox obstacle list for layout-side label placement.

    Mirrors ``nf_metro.render.svg._compute_icon_obstacles`` using
    layout-side constants (``TERMINUS_WIDTH``, ``ICON_HALF_HEIGHT``)
    so we don't depend on the render layer or theme defaults.
    """
    half_w = TERMINUS_WIDTH / 2
    half_h = ICON_HALF_HEIGHT
    caption_extent = ICON_CAPTION_GAP + ICON_CAPTION_FONT_HEIGHT
    obstacles: list[tuple[float, float, float, float]] = []
    for st in graph.stations.values():
        if not st.is_terminus or st.is_hidden:
            continue
        names = st.terminus_names or []
        bot_extra = caption_extent if any(bool(n) for n in names) else 0.0
        obstacles.append(
            (
                st.x - half_w,
                st.y - half_h,
                st.x + half_w,
                st.y + half_h + bot_extra,
            )
        )
    return obstacles
