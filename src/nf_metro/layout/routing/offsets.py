...
def _reconcile_horizontal_offsets(ctx: _OffsetCtx, max_iterations: int = 10) -> None:
    """Snap offsets for same-section, same-Y edges so segments stay flat.

    Only processes edges where both endpoints belong to the same
    section. Inter-section reconciliation cascades offsets across
    section boundaries and breaks per-section reindexing.
