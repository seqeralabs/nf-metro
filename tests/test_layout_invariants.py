...
@pytest.mark.parametrize("fixture", _SYMMETRIC_DEADEND_FANOUT_FIXTURES)
def test_symmetric_fork_entry_port_stays_on_feeder_trunk(fixture):
    """A symmetric fork fans around its entry port without moving it.

    The branches straddle the port; the port itself stays on the inter-section
    trunk arriving from the feeding section (its feeder's exit Y), so the run
    between the two sections is straight and the row's trunk keeps its Y.  A fork
    that instead recentres the port onto the fork midline drags the whole row
    off that trunk.
    """
    ...
    for section in graph.sections.values():
        ...
        feeder_ys = {
            ...
            if (src := graph.stations.get(e.source)) is not None and src.is_port
        }
        for fy in feeder_ys:
            assert abs(graph.stations[entry_port].y - fy) < 1.0, (
                f"{fixture}: {section.id} entry port sits at "
                f"y={graph.stations[entry_port].y:.1f} but its feeder arrives at "
                f"y={fy:.1f} -- the fork dragged the port off the trunk"
            )
            checked += 1
