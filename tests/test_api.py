...
def test_prepare_graph_accepts_a_parsed_graph() -> None:
    ...
    Confirms the deepcopy-and-layout split the CLI relies on (one parse
    shared across several output formats) never changes what gets drawn.
    """
    from nf_metro.api import _parse_source, render_graph
    from nf_metro.themes import resolve_theme

    text = (EXAMPLES / "rnaseq_auto.mmd").read_text()
    source_dir = str(EXAMPLES)

    direct = prepare_graph(text, source_dir=source_dir)
    via_parse = prepare_graph(_parse_source(text), source_dir=source_dir)
